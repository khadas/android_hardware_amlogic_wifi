...
static int nl80211_set_iface_id(struct nl_msg *msg, struct i802_bss *bss)
{
	if (bss->wdev_id_set) {
		...
		wpa_printf(MSG_DEBUG, "%s: NL80211_ATTR_WDEV 0x%llx", __FUNCTION__, (long long unsigned int) bss->wdev_id);
	} else {
		NLA_PUT_U32(msg, NL80211_ATTR_IFINDEX, bss->ifindex);
		wpa_printf(MSG_DEBUG, "%s: NL80211_ATTR_IFINDEX %d", __FUNCTION__, bss->ifindex);
	}
	return 0;
...
static int process_drv_event(struct nl_msg *msg, void *arg)
{
	...
	if (tb[NL80211_ATTR_IFINDEX]) {
		...
		wpa_printf(MSG_DEBUG, "%s: NL80211_ATTR_IFINDEX %d", __FUNCTION__, ifidx);
		...
		wpa_printf(MSG_DEBUG, "%s: NL80211_ATTR_WDEV 0x%llx", __FUNCTION__, (long long unsigned int) wdev_id);
		for (bss = &drv->first_bss; bss; bss = bss->next) {
...
static void nl80211_del_p2pdev(struct i802_bss *bss)
{
	...
	wpa_printf(MSG_DEBUG, "nl80211: Delete P2P Device %s (wdev 0x%llx): %s",
...
static int nl80211_set_p2pdev(struct i802_bss *bss, int start)
{
	...
	wpa_printf(MSG_DEBUG, "nl80211: %s P2P Device %s (wdev 0x%llx): %s",
		   start ? "Start" : "Stop",
		   bss->ifname, (long long unsigned int) bss->wdev_id,
		   strerror(ret));
...
#ifndef CONFIG_P2P_HACK_PRE38
	if (wpa_driver_nl80211_capa(drv))
		return -1;
#endif /* !CONFIG_P2P_HACK_PRE38 */
...
/*
 * XXX:
 * FIXME: In case of dynamically added interface (e.g., P2P), don't set
 *        the interface mode unless driver capabilities are updated.
 */
		    ...
		    struct wpa_driver_scan_params *params, u64 *wdev_id)
{
	...
	if (!wdev_id) {
		NLA_PUT_U32(msg, NL80211_ATTR_IFINDEX, drv->ifindex);
		wpa_printf(MSG_DEBUG, "%s: NL80211_ATTR_IFINDEX %d", __FUNCTION__, drv->ifindex);
	} else {
...
nl80211_get_scan_results(struct wpa_driver_nl80211_data *drv)
{
	...
	nl80211_cmd(drv, msg, NLM_F_DUMP, NL80211_CMD_GET_SCAN);
	wpa_printf(MSG_DEBUG, "%s: ifindex %d bss ifindex %d bss wdev 0x%llx", __FUNCTION__, drv->ifindex, drv->first_bss.ifindex, (long long unsigned int) drv->first_bss.wdev_id);
	if (nl80211_set_iface_id(msg, &drv->first_bss) < 0)
		goto nla_put_failure;
...
static int nl80211_wdev_handler(struct nl_msg *msg, void *arg)
{
	...
	if (tb[NL80211_ATTR_WDEV]) {
		...
		wi->wdev_id_set = 1;
		wpa_printf(MSG_DEBUG, "%s: NL80211_ATTR_WDEV 0x%llx", __FUNCTION__, (long long unsigned int) wi->wdev_id);
	}
				     ...
				     const char *bridge)
{
	...
	if (nlmode == NL80211_IFTYPE_P2P_DEVICE) {
		...
		wpa_printf(MSG_DEBUG, "nl80211: New P2P Device interface %s (wdev 0x%llx) created",
			   ifname,
