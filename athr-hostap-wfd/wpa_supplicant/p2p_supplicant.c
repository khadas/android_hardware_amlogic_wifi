...
static void wpas_p2p_fallback_to_go_neg(struct wpa_supplicant *wpa_s,
					int group_added);
static void wpas_wfd_ie_print(struct wpa_supplicant *wpa_s, u8 *addr, u8 frame_type);
...
static void wpas_p2p_group_delete(struct wpa_supplicant *wpa_s, int silent)
{
	...
	if (wpa_drv_probe_req_report(wpa_s, 0) < 0) {
...
#ifdef CE_CUSTOM_0
	wpa_msg_ctrl(wpa_s, MSG_INFO, WPA_EVENT_SYNC);
#endif /* CE_CUSTOM_0 */
...
static void p2p_go_configured(void *ctx, void *data)
{
	...
	if (params->wps_method == WPS_PBC)
		wpa_supplicant_ap_wps_pbc(wpa_s, params->peer_interface_addr,
					 //NULL);//+FLUG20120524
					 params->peer_device_addr);//-FLUG20120524
...
wpas_p2p_init_group_interface(struct wpa_supplicant *wpa_s, int go)
{
	...
	if (!go) {
		struct wps_context *wps = group_wpa_s->wps;
		if (!wps->dev.device_name) {
			wps->dev.device_name = group_wpa_s->conf->device_name;
			wps->dev.manufacturer = group_wpa_s->conf->manufacturer;
			wps->dev.model_name = group_wpa_s->conf->model_name;
			wps->dev.model_number = group_wpa_s->conf->model_number;
			wps->dev.serial_number = group_wpa_s->conf->serial_number;
			group_wpa_s->wps->config_methods = wpa_s->wps->config_methods;
			wps->dev.config_methods = wps->config_methods;
			os_memcpy(wps->dev.pri_dev_type, group_wpa_s->conf->device_type,
			          WPS_DEV_TYPE_LEN);
			wps->dev.num_sec_dev_types = group_wpa_s->conf->num_sec_device_types;
			os_memcpy(wps->dev.sec_dev_type, group_wpa_s->conf->sec_device_type,
			          WPS_DEV_TYPE_LEN * wps->dev.num_sec_dev_types);
			wps->dev.os_version = WPA_GET_BE32(group_wpa_s->conf->os_version);
		}
	}

	if (go && wpa_s->conf->p2p_max_clients) {
		wpa_printf(MSG_DEBUG, "P2P: p2p maximum clients %d", wpa_s->conf->p2p_max_clients);
		wpa_drv_set_p2p_max_clients(group_wpa_s, wpa_s->conf->p2p_max_clients);
	}

	/* FIX */
	wpa_printf(MSG_DEBUG, "P2P: set scanmode %d to %s", wpa_s->conf->p2p_scan_band,
							(go ? "GO" : "P2P Client"));
	if (wpa_s->conf->p2p_scan_band == 1)
		wpa_drv_driver_cmd(group_wpa_s, "scanmode skip5g", NULL, 0);
	else if (wpa_s->conf->p2p_scan_band == 2)
		wpa_drv_driver_cmd(group_wpa_s, "scanmode skip2g", NULL, 0);
	else
        	wpa_drv_driver_cmd(group_wpa_s, "scanmode all", NULL, 0);
					     ...
					     void *timeout_ctx)
{
	...
	struct wpa_supplicant *group_wpa_s = wpa_s->global->p2p_group_formation;

	wpa_printf(MSG_DEBUG, "P2P: Group Formation timed out");

	if (group_wpa_s && group_wpa_s->p2p_in_provisioning)
		wpa_msg(group_wpa_s->parent, MSG_INFO, WPS_EVENT_TIMEOUT);
...
void wpas_go_neg_completed(void *ctx, struct p2p_go_neg_results *res)
{
	...
	if (wpa_s->global->p2p)
	    wpas_wfd_ie_print(wpa_s, res->peer_device_addr, 0);
	wpas_notify_p2p_go_neg_completed(wpa_s, res);
		    ...
		    int new_device)
{
	...
	if (info->wfd_subelems) {
		char *pos, *end, wfd_subelems[WFD_SUBELEMS_LEN];
		int res;

		os_memset(wfd_subelems, 0, WFD_SUBELEMS_LEN);
		pos = wfd_subelems;
		end = wfd_subelems + WFD_SUBELEMS_LEN;
		...
		pos += res;
		wpa_msg(wpa_s, MSG_INFO, "%s", wfd_subelems);
	}
					 ...
					 unsigned int generated_pin)
{
	...
	if (wpa_s->global->p2p)
	    wpas_wfd_ie_print(wpa_s, peer, 0);
}
			...
			size_t group_id_len)
{
	...
	if (wpa_s->global->p2p)
	    wpas_wfd_ie_print(wpa_s, peer, WFD_IE_FROM_PROV_REQ);
...
void wpas_prov_disc_resp(void *ctx, const u8 *peer, u16 config_methods)
{
	...
	if (wpa_s->global->p2p)
	    wpas_wfd_ie_print(wpa_s, peer, WFD_IE_FROM_PROV_RESP);
...
static u8 wpas_invitation_process(void *ctx, const u8 *sa, const u8 *bssid,
				  ...
				  int *force_freq, int persistent_group, int authorized)
{
	struct wpa_supplicant *wpa_s = ctx;
	struct wpa_ssid *s;
	...
	if (!wpa_s->conf->persistent_reconnect && !authorized)
		return P2P_SC_FAIL_INFO_CURRENTLY_UNAVAILABLE;
	...
	if (status == P2P_SC_SUCCESS) {
		...
		if (s) {
			wpas_p2p_group_add_persistent(
				wpa_s, s, s->mode == WPAS_MODE_P2P_GO, op_freq);
...
static void wpas_invitation_result(void *ctx, int status, const u8 *bssid, int *op_freq)
{
	struct wpa_supplicant *wpa_s = ctx;
	struct wpa_ssid *ssid;
	...
	{	/* check go exist or not */
		...
		grp = wpas_get_p2p_group(wpa_s, ssid, ssid->ssid_len, &go);
		if (grp != NULL)
			*op_freq = 0;
	}
	wpas_p2p_group_add_persistent(wpa_s, ssid,
				      ssid->mode == WPAS_MODE_P2P_GO, *op_freq);
}
...
int wpas_p2p_init(struct wpa_global *global, struct wpa_supplicant *wpa_s)
{
	...
	p2p.persistent_reconnect = wpa_s->conf->persistent_reconnect;
	p2p.p2p_intra_bss = wpa_s->conf->p2p_intra_bss;

	/* FIX */
	p2p.p2p_scan_band = wpa_s->conf->p2p_scan_band;
	wpa_printf(MSG_DEBUG, "P2P: set scanmode %d to p2pdevice", wpa_s->conf->p2p_scan_band);
	if (wpa_s->conf->p2p_scan_band == 1)
		wpa_drv_driver_cmd(wpa_s, "scanmode skip5g", NULL, 0);
	else if (wpa_s->conf->p2p_scan_band == 2)
		wpa_drv_driver_cmd(wpa_s, "scanmode skip2g", NULL, 0);
	else
		wpa_drv_driver_cmd(wpa_s, "scanmode all", NULL, 0);

	p2p.p2p_expire_age = wpa_s->conf->p2p_expire_age;
	if (p2p.p2p_expire_age == 0)
		p2p.p2p_expire_age = DEFAULT_P2P_PEER_EXPIRATION_AGE;
	wpa_printf(MSG_DEBUG, "P2P: set p2p expiration age %d seconds", p2p.p2p_expire_age);

	p2p.p2p_expire_poll = wpa_s->conf->p2p_expire_poll;
	if (p2p.p2p_expire_poll == 0)
		p2p.p2p_expire_poll = DEFAULT_P2P_PEER_EXPIRATION_INTERVAL;
	wpa_printf(MSG_DEBUG, "P2P: set p2p expiration poll interval %d seconds", p2p.p2p_expire_poll);

	p2p.p2p_neg_conf_ack = wpa_s->conf->p2p_neg_conf_ack;
		     ...
		     int go_intent, int freq, int persistent_id, int pd)
{
	...
	if (freq > 0) {
		...
		if (freq == 2) {
			wpa_printf(MSG_DEBUG, "P2P: Request to set operation channel on 2.4 GHz "
			           "band");
			...
			wpa_printf(MSG_DEBUG, "P2P: Request to set operation channel on 5 GHz "
			           "band");
		...
		if (acs_enabled) {
			if (oper_freq > 0 &&
				...
				wpa_printf(MSG_DEBUG, "P2P: Start P2P group "
				           "on the current operating "
				           "channel (%u MHz)", oper_freq);
				freq = oper_freq; /* force to use the current operating channel */
				acs_enabled = 0;
			}	
		}
		...
		wpa_printf(MSG_DEBUG, "P2P: %s choose operating channel from best channels",
		           (acs_enabled ? "" : "NOT"));
		wpas_p2p_set_acs(wpa_s, acs_enabled);
	} else if (oper_freq > 0 &&
	...
	peer_info = p2p_wfd_get_peer_info(wpa_s->global->p2p, peer_addr);
	if (peer_info == NULL) {
		wpa_msg(wpa_s->parent, MSG_INFO, "P2P: Peer is not Found");
		return -1;
	} else {
		if (p2p_wfd_device_type_check(wpa_s->global->p2p,
		                              peer_info->wfd_subelems)) {
			wpa_msg(wpa_s->parent, MSG_INFO, "WFD-IE Device Type Mismatched");
			return -1;
		}
	}
			 ...
			 int go)
{
	...
	if (!wpas_p2p_create_iface(wpa_s)) {
		//WAR for Mck,we need to set p2p_max_client here,UB94?
		if(go && wpa_s->conf->p2p_max_clients) {
			wpa_printf(MSG_DEBUG, "P2P: p2p maximum clients %d",wpa_s->conf->p2p_max_clients);
			wpa_drv_set_p2p_max_clients(wpa_s, wpa_s->conf->p2p_max_clients);
		}
		return wpa_s;
	}
		       ...
		       int freq)
{
	...
	if (freq > 0) {
		...
		if (oper_freq > 0 &&
		    freq != oper_freq &&
		    !(wpa_s->drv_flags & WPA_DRIVER_FLAGS_MULTI_CHANNEL_CONCURRENT)) {
			wpa_printf(MSG_DEBUG, "P2P: Create P2P group "
			           "on the current operating "
			           "channel (%u MHz)", oper_freq);
			freq = oper_freq; /* force to use the current operating channel */
		}
		...
		if (freq > 0 && !p2p_supported_freq(wpa_s->global->p2p, freq)) {
			...
			return -1;
	           ...
	           !p2p_supported_freq(wpa_s->global->p2p, oper_freq)) {
		if (!(wpa_s->drv_flags & WPA_DRIVER_FLAGS_MULTI_CHANNEL_CONCURRENT)) {
			wpa_printf(MSG_DEBUG, "P2P: Cannot Create P2P group "
			           "while connected on non-P2P supported "
			           "channel (%u MHz)", oper_freq);
			return -1;
		}
		wpa_printf(MSG_DEBUG, "P2P: Current operating channel "
		           "(%u MHz) not available for P2P creation - try to use "
		           "another channel", oper_freq);
		freq = 0;
	} else if (oper_freq > 0) {
		wpa_printf(MSG_DEBUG, "P2P: Trying to force us to use the "
				  ...
				  int freq)
{
	...
	if (freq > 0) {
		if (freq == 2) {
			wpa_printf(MSG_DEBUG, "P2P: Request to start persistent GO on 2.4 GHz "
			           "band");
			if (wpa_s->best_24_freq > 0 &&
				p2p_supported_freq(wpa_s->global->p2p,
				                   wpa_s->best_24_freq)) {
 		...
 		if (freq == 5) {
			wpa_printf(MSG_DEBUG, "P2P: Request to start persistent GO on 5 GHz "
			           "band");
			if (wpa_s->best_5_freq > 0 &&
				p2p_supported_freq(wpa_s->global->p2p,
				                   wpa_s->best_5_freq)) {
		...
		if (oper_freq > 0 &&
		    freq != oper_freq &&
		    !(wpa_s->drv_flags & WPA_DRIVER_FLAGS_MULTI_CHANNEL_CONCURRENT)) {
			wpa_printf(MSG_DEBUG, "P2P: Create P2P persistent group "
			           "on the current operating "
			           "channel (%u MHz)", oper_freq);
			freq = oper_freq; /* force to use the current operating channel */
		}
		...
		if (freq > 0 && !p2p_supported_freq(wpa_s->global->p2p, freq)) {
			wpa_printf(MSG_DEBUG, "P2P: The forced channel for persistent GO "
			           "(%u MHz) is not supported for P2P uses",
			           freq);
			return -1;
		}
	           ...
	           !p2p_supported_freq(wpa_s->global->p2p, oper_freq)) {
		if (!(wpa_s->drv_flags & WPA_DRIVER_FLAGS_MULTI_CHANNEL_CONCURRENT)) {
			wpa_printf(MSG_DEBUG, "P2P: Cannot Create persistent P2P group "
			           "while connected on non-P2P supported "
			           "channel (%u MHz)", oper_freq);
			return -1;
		}
		wpa_printf(MSG_DEBUG, "P2P: Current operating channel "
		           "(%u MHz) not available for P2P persistent group creation - try to use "
		           "another channel", oper_freq);
		freq = 0;
	} else if (oper_freq > 0) {
		wpa_printf(MSG_DEBUG, "P2P: Trying to force us to use the "
			 ...
			 struct wps_event_fail *fail)
{
	...
	if ((i = eloop_cancel_timeout(wpas_p2p_group_formation_timeout, wpa_s, NULL)) > 0) {
		eloop_register_timeout(1, 0, wpas_p2p_group_formation_timeout, wpa_s, NULL);
	}
                  ...
                  const u8 *dev_id)
{
	...
	p2p_clean_rssi(wpa_s->global->p2p);
	...
	if (wpa_drv_probe_resp_report(wpa_s, 1) < 0) {
		wpa_printf(MSG_DEBUG, "P2P: Failed to request the driver to "
			   "report received Probe Response frames");
		return -1;
	}
...
{
	if (wpa_s->driver && wpa_s->drv_priv) {
		wpa_drv_probe_resp_report(wpa_s, 0);
	}
...
{
	struct wpabuf *probe_resp_ie = NULL;
	...
	if (p2p_ext_listen(wpa_s->global->p2p, period, interval))
		return -1;

	probe_resp_ie = p2p_build_probe_resp_ies(wpa_s->global->p2p, NULL);
...
int wpas_p2p_set_go_apsd(struct wpa_supplicant *wpa_s)
{
	int enable;
	...
	if (wpa_s->set_ap_uapsd && wpa_s->ap_uapsd) {
		enable = 1;
	}  else {
		enable = 0;
	}

	if (!wpa_drv_ap_set_uapsd(wpa_s, enable)) {
		hostapd_p2p_set_go_apsd(wpa_s->ap_iface->bss[0], enable);
	} else {
		return -1;
	}
...
int wpas_p2p_set_group_limit(struct wpa_supplicant *wpa_s, int limit_reached)
{
	wpa_printf(MSG_DEBUG, "P2P: set group limit flag %s",limit_reached==1?"on":"off");
	return p2p_group_set_group_limit(wpa_s->p2p_group, limit_reached);
}


int wpas_p2p_set_acs(struct wpa_supplicant *wpa_s, int acs_state)
{
	struct p2p_data *p2p = wpa_s->global->p2p;

	wpa_printf(MSG_DEBUG, "P2P: set acs %d", acs_state);
	return p2p_set_acs_state(p2p, acs_state);
}


int wpas_p2p_dump_rssi(struct wpa_supplicant *wpa_s)
{
	return p2p_dump_rssi(wpa_s->global->p2p);
}


void wpas_p2p_set_rssi_thr(struct wpa_supplicant *wpa_s, int rssi_thr)
{
	p2p_set_rssi_thr(wpa_s->global->p2p, rssi_thr); 
}
...
{
    struct p2p_data *p2p = wpa_s->global->p2p;
	char wfd_subelems[WFD_SUBELEMS_LEN];
	int ret = 0;

	os_memset(wfd_subelems, 0, WFD_SUBELEMS_LEN);
	ret = p2p_get_peer_wfd_ie(p2p, addr, frame_type, wfd_subelems);
	if (ret > 0)
		wpa_msg(wpa_s, MSG_INFO, "%s", wfd_subelems);
}
