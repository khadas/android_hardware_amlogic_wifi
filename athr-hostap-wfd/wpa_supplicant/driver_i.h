...
static inline int wpa_drv_probe_resp_report(struct wpa_supplicant *wpa_s,
					   int report)
{
	if (wpa_s->driver->probe_resp_report)
		return wpa_s->driver->probe_resp_report(wpa_s->drv_priv,
						       report);
	return -1;
}

static inline int wpa_drv_set_tx_sgi(struct wpa_supplicant *wpa_s,
					u32* mask, u8 threshold)			
{
	if (wpa_s->driver->set_tx_sgi)
		return wpa_s->driver->set_tx_sgi(wpa_s->drv_priv, mask, threshold);
	return -1;
}

static inline int wpa_drv_ap_set_uapsd(struct wpa_supplicant *wpa_s,
				       int enabled)
{
	if (wpa_s->driver->ap_set_uapsd)
		return wpa_s->driver->ap_set_uapsd(wpa_s->drv_priv, enabled);
	return -1;
}
...
static inline int wpa_drv_kick_sta(struct wpa_supplicant *wpa_s, u8 *addr)
{
	if (wpa_s->driver->kick_sta)
		return wpa_s->driver->kick_sta(wpa_s->drv_priv, addr);
	return -1;
}
...
static inline int wpa_drv_set_p2p_max_clients(struct wpa_supplicant *wpa_s, int max_clients)
{
	if(wpa_s->driver->set_p2p_max_clients)
		return wpa_s->driver->set_p2p_max_clients(wpa_s->drv_priv, max_clients);
	return -1;
}

static inline const char * wpa_drv_get_radio_name(struct wpa_supplicant *wpa_s)
{
...
static inline int wpa_drv_get_scanband(struct wpa_supplicant *wpa_s, u32 *scanband)
{
	if (wpa_s->driver->get_scanband) {
		return wpa_s->driver->get_scanband(wpa_s->drv_priv, scanband);
	}
	return -1;
}
#endif /* DRIVER_I_H */
