...
static void wpas_send_action_cb(void *eloop_ctx, void *timeout_ctx)
{
	...
	if (p2p_check_prov_pending(wpa_s->global->p2p) &&
	    (wpa_s->off_channel_freq == 0 ||
	     wpa_s->off_channel_freq == wpa_s->pending_action_freq) &&
	    (!(wpa_s->drv_flags &
	       WPA_DRIVER_FLAGS_MULTI_CHANNEL_CONCURRENT))) {
		struct wpa_supplicant *tmp_wps;
		struct wpa_global *global = wpa_s->global;
		int do_speed_up_flag = 1;

		for (tmp_wps = global->ifaces; tmp_wps; tmp_wps = tmp_wps->next) {
			if (tmp_wps->scanning == 1) {
				int scanband;
				if (wpa_drv_get_scanband(tmp_wps,&scanband) == 0) {
					if (scanband > 2) {//single channel scan,skip this scan check
						//printf("%s[%d]scanband=%d\n\r",__func__,__LINE__,scanband);
						continue;
					}
				}				
				if (wpa_s->reassociate == 1){//reassociate,skip this check
					//printf("%s[%d]reassociate=%d\n\r",__func__,__LINE__,wpa_s->reassociate);
					continue;
				}
				do_speed_up_flag = 0;
				//printf("%s[%d]\n\r",__func__,__LINE__);
		...
		if (do_speed_up_flag == 1) {
			for (tmp_wps = global->ifaces; tmp_wps; tmp_wps = tmp_wps->next) {
				if (tmp_wps->p2p_group_interface == P2P_GROUP_INTERFACE_GO &&
				    tmp_wps->wpa_state >= WPA_ASSOCIATED) {
					if(wpa_s->pending_action_freq == tmp_wps->assoc_freq) {
						goto out; 
					}
				}
			}
			   ...
			   int no_cck)
{
	...
	/* Speed up Provision resp */
	if (freq &&
	    p2p_check_prov_pending(wpa_s->global->p2p) &&
	    wpa_s->off_channel_freq == 0 &&
	    (!(wpa_s->drv_flags &
	       WPA_DRIVER_FLAGS_MULTI_CHANNEL_CONCURRENT))) {
		struct wpa_supplicant *tmp_wps;
		...
		for (tmp_wps = global->ifaces; tmp_wps; tmp_wps = tmp_wps->next) {
			if (tmp_wps->scanning == 1) {
				int scanband;
				if (wpa_drv_get_scanband(tmp_wps,&scanband) == 0) {
					if ((scanband > 2) && (freq == scanband)) {//single channel scan,skip this scan check
						//printf("%s[%d]scanband=%d\n\r",__func__,__LINE__,scanband);
						continue;
					}
				}
				...
				do_speed_up_flag = 0;
				wpa_printf(MSG_DEBUG,"%s[%d]tmp_wps->ifname=%s,freq=%d,%d,scanband=%d\n\r",__func__,__LINE__,tmp_wps->ifname,freq,wpa_s->off_channel_freq,scanband);
				break;
			}
		}
		if (do_speed_up_flag == 1) {
			for (tmp_wps = global->ifaces; tmp_wps; tmp_wps = tmp_wps->next) {
				//printf("%s[%d]tmp_wps->ifname=%s,%d,%d,%d,%d\n\r", __func__, __LINE__, tmp_wps->ifname, tmp_wps->p2p_group_interface,tmp_wps->wpa_state,freq,tmp_wps->assoc_freq);
				if (tmp_wps->p2p_group_interface == P2P_GROUP_INTERFACE_GO &&
					tmp_wps->wpa_state >= WPA_ASSOCIATED) {
					if (freq == tmp_wps->assoc_freq) {
						//printf("%s[%d]No ROC\n\r", __func__, __LINE__);
						freq = 0;
						break; 
					}
		...
		if (freq != 0) {
			wpa_printf(MSG_DEBUG,"%s[%d]do ROC for send prov resp\n\r",__func__,__LINE__);
		}
	}
