					    ...
					    struct wpa_ssid *group)
{
	...
	for (ssid = group; ssid; ssid = ssid->pnext) {
		...
		if (!wpa_supplicant_ssid_bss_match(wpa_s, ssid, bss)) {
			/* FIX : Reset to 1 to always do signal channel scan. */
			if (ssid->bssid_set) {
...
#ifdef CONFIG_UPDATE_SIBLING_SCANRESULT
	/*
					  ...
					  int locally_generated)
{
	...
	if ((wpa_s->prev_wpa_state == WPA_4WAY_HANDSHAKE &&
	     wpa_s->wpa_state == WPA_DISCONNECTED &&
	     wpa_s->p2p_group_interface == P2P_GROUP_INTERFACE_CLIENT) &&
	     wpa_key_mgmt_wpa_psk(wpa_s->key_mgmt)) {
		wpa_msg(wpa_s->parent, MSG_INFO, "WPA: 4-Way Handshake failed - "
		        "pre-shared key may be incorrect");
	}
...
#ifndef CONFIG_NO_STDOUT_DEBUG
{
	...
	case EVENT_RX_MGMT: {
			...
			if (stype == WLAN_FC_STYPE_PROBE_RESP && (data->rx_mgmt.frame_len > 24)) {
				const u8 *ie = mgmt->u.probe_resp.variable;
				size_t ie_len = data->rx_mgmt.frame_len -
				                (mgmt->u.probe_resp.variable -
				                data->rx_mgmt.frame);
				if (p2p_dev_addr_get_from_ie(&assign_mac, ie, ie_len) != 0) {
					memcpy(assign_mac,mgmt->sa,6); //use src mac
				}

				p2p_update_device_rssi(wpa_s->global->p2p,
				                       &assign_mac,
	...
	case EVENT_P2P_SD_RESPONSE:
		...
		break;
	case EVENT_SET_STA_LIMIT:
		wpas_p2p_set_group_limit(wpa_s, 1);
		break;
	case EVENT_CLEAR_STA_LIMIT:
		wpas_p2p_set_group_limit(wpa_s, 0);
		break;
	case EVENT_ACL_REJECT:
		wpa_msg(wpa_s, MSG_INFO, WPA_EVENT_STA_REJECTED "STA="
					MACSTR, MAC2STR(data->acl_reject_info.addr));
		break;
#endif /* CONFIG_P2P */
	case EVENT_EAPOL_RX:
