...
ssid_list_set:
	...
	if(params.freqs == NULL &&
		...
		for (tmp_wps = global->ifaces; tmp_wps; tmp_wps = tmp_wps->next) {
			if((tmp_wps == wpa_s) ||
				(os_strcmp(wpa_drv_get_radio_name(wpa_s), wpa_drv_get_radio_name(tmp_wps)) != 0))
				continue;

			if (tmp_wps->wpa_state >= WPA_ASSOCIATED) {
				params.freqs = os_zalloc(2 * sizeof(int));
				if (params.freqs)
					params.freqs[0] = tmp_wps->assoc_freq;
				break;           
			}
		}
		...
		if(params.freqs) { 
			...
			wpa_printf(MSG_DEBUG, "Single channel operation, scan only frequency %d MHz",
			params.freqs[0]);
		} else if (wpa_s->next_scan_freqs) {
			wpa_dbg(wpa_s, MSG_DEBUG, "Not concurrent, optimize scan based on previously "
			"generated frequency list");
			params.freqs = wpa_s->next_scan_freqs;
		} else
			os_free(wpa_s->next_scan_freqs);
	} else if (params.freqs == NULL && wpa_s->next_scan_freqs && scan_req != 2) {
