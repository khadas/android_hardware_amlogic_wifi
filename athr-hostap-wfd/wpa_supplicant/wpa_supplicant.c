...
void WIDI_set_state_up(struct wpa_supplicant *wpa_s, int state)
{
	...
	if(wpa_s->global->widi_sock_l2sd)
    	sendto(wpa_s->global->widi_sock_l2sd, &l2_notif, sizeof(l2_notif), 0,(struct sockaddr*)&local, sizeof(local));
	else
...
void WIDI_set_source_info(struct wpa_supplicant *wpa_s)
{
	...
	strcpy(l2_notif.u.l2sd_QCA_profile_sync_msg.passphrase, wpa_s->current_ssid->passphrase);
			      ...
			      enum wpa_states state)
{
	...
	wpa_s->prev_wpa_state = old_state;
			      ...
			      struct wpa_bss *bss, struct wpa_ssid *ssid)
{
	...
	if (ssid->mode == WPAS_MODE_AP || ssid->mode == WPAS_MODE_P2P_GO ||
		...
		if (ssid->p2p_persistent_group &&
		    wpa_s->global->wpa_psk_file_path &&
		    wpa_s->parent->conf->wpa_psk_file) {
			ssid->wpa_psk_file = wpa_psk_file_name_get(wpa_s->parent->conf->wpa_psk_file,
			                                           ssid->ssid);
			if (!ssid->wpa_psk_file) {
				wpa_printf(MSG_DEBUG, "Failed to allocate memory for wpa_psk_file");
				return;
			}
			ssid->wpa_psk_file_path = os_strdup(wpa_s->global->wpa_psk_file_path);
			f_name = wpa_psk_file_path_name_get(ssid->wpa_psk_file_path,
			                                    ssid->wpa_psk_file,
			                                    ssid->ssid);
			if (f_name == NULL) {
				wpa_printf(MSG_DEBUG, "Failed to allocate memory for full path and name of wpa_psk_file");
				return;
			}

			f = fopen(f_name, "r");
			if (f == NULL) {
				wpa_printf(MSG_DEBUG, "Need to create New PSK File - '%s'", f_name);
				f = fopen(f_name, "a");
				if (f == NULL) {
					wpa_printf(MSG_DEBUG, "Failed to create New PSK File - '%s'", f_name);
					os_free(f_name);
					return;
				}
			} else {
				wpa_printf(MSG_DEBUG, "PSK File Existed - '%s'", f_name);
			}
			fclose(f);
			os_free(f_name);

			wpa_printf(MSG_DEBUG, "wpa_psk_file_path : '%s'", ssid->wpa_psk_file_path);
			wpa_printf(MSG_DEBUG, "wpa_psk_file : '%s'", ssid->wpa_psk_file);
		}
		...
		if (ssid->p2p_persistent_group &&
		    ssid->wpa_psk_file_path &&
		    ssid->wpa_psk_file) {
			if(wpa_s->ap_iface != NULL) {
				for (i = 0; i < wpa_s->ap_iface->num_bss; i++) {
					struct hostapd_data *hapd = wpa_s->ap_iface->bss[i];
					struct hostapd_ssid *h_ssid = &hapd->conf->ssid;

					if (os_memcmp(h_ssid->ssid, ssid->ssid, ssid->ssid_len) == 0) {
						hapd->wps->psk_per_sta = 1;
						break;
					}
				}
			}
	...
	if (bss && (wpa_bss_get_vendor_ie(bss, WPA_IE_VENDOR_TYPE) ||
		...
		if (os_strstr(ssid->eap.phase1, "pbc=1"))
			pbc = 1;
		wpa_printf(MSG_DEBUG, "WPA: Set WPS IE Device PWD %d", pbc);
		wps_ie = wps_build_assoc_req_ie(pbc, wpas_wps_get_req_type(ssid));
...
static const u8 *wpas_sta_get_p2p_dev_addr(struct hostapd_data *hapd, const u8 *peer)
{
	struct sta_info *sta_list = hapd->sta_list;

	while (sta_list) {
		if (os_memcmp(sta_list->addr, peer, ETH_ALEN) == 0)
			return sta_list->p2p_dev_addr;
...
void wpas_remove_psk(struct wpa_supplicant *wpa_s, const u8 *peer)
{
	int i;
	char *f_name;
	struct hostapd_data *hapd;
	struct hostapd_ssid *ssid;
	struct hostapd_wpa_psk *p, *current_p = NULL;

	for (i = 0; i < wpa_s->ap_iface->num_bss; i++) {
		hapd = wpa_s->ap_iface->bss[i];
		ssid = &hapd->conf->ssid;
		if (hapd->wps->psk_per_sta && ssid->wpa_psk_file) {
			for (p = ssid->wpa_psk; p; p = p->next) {
				if (is_zero_ether_addr(p->addr)) {
					current_p = p;
					continue;
				}
				if (os_memcmp(p->addr, peer, ETH_ALEN) == 0) {
					if (current_p == NULL)
						ssid->wpa_psk = p->next;
					else
			...
			if (wpa_psk_file_update(f_name, peer, NULL))
				wpa_printf(MSG_DEBUG, "Per-Key remove failed");
...
static void wpas_update_passphrase(struct wpa_ssid *ssid, struct hostapd_ssid *h_ssid)
{
	struct wpa_ssid *s;

	for (s = ssid; s; s = s->next) {
		if ((s->ssid_len == h_ssid->ssid_len) &&
		    (os_memcmp(s->ssid, h_ssid->ssid, s->ssid_len) == 0)) {
			if (s->passphrase) {
				os_free(s->passphrase);
				s->passphrase = os_strdup(h_ssid->wpa_passphrase);
			}
			break;
		}
	}
...
void wpas_refresh_psk(struct wpa_supplicant *wpa_s, char *passphrase)
{
	struct wpa_ssid *cur_ssid = wpa_s->current_ssid;
	struct hostapd_data *hapd;
	struct hostapd_ssid *h_ssid;
	struct hostapd_wpa_psk *p;
	int i, is_found = 0, is_non_wps_legacy;
	struct sta_info *sta_list;

	for (i = 0; i < wpa_s->ap_iface->num_bss; i++) {
		hapd = wpa_s->ap_iface->bss[i];
		h_ssid = &hapd->conf->ssid;
		if (os_memcmp(h_ssid->ssid, cur_ssid->ssid, cur_ssid->ssid_len) == 0) {
			os_free(h_ssid->wpa_passphrase);
			h_ssid->wpa_passphrase = os_strdup(passphrase);
			for (p = h_ssid->wpa_psk; p; p = p->next) {
				if (is_zero_ether_addr(p->addr)) {
					pbkdf2_sha1(h_ssid->wpa_passphrase,
					            h_ssid->ssid, h_ssid->ssid_len,
					            4096, p->psk, PMK_LEN);
					break;
				}
			}

			os_free(cur_ssid->passphrase);
			cur_ssid->passphrase = os_strdup(passphrase);
			is_found = 1;
			break;
		}
	}

	if (is_found && cur_ssid->p2p_persistent_group) {
		wpas_update_passphrase(wpa_s->conf->ssid, h_ssid);
		wpas_update_passphrase(wpa_s->parent->conf->ssid, h_ssid);

#ifndef CONFIG_NO_CONFIG_WRITE
		if (wpa_config_write(wpa_s->parent->confname, wpa_s->parent->conf))
	...
	for(sta_list = hapd->sta_list; sta_list; sta_list = sta_list->next) {
		is_non_wps_legacy = 1;
		for (p = h_ssid->wpa_psk; p; p = p->next) {
			if (os_memcmp(sta_list->addr, p->addr, ETH_ALEN) == 0) {
				is_non_wps_legacy = 0;
				break;
			}
		}
		if (is_non_wps_legacy && is_zero_ether_addr(sta_list->p2p_dev_addr))
			wpa_drv_kick_sta(wpa_s, sta_list->addr);
	}
}

int wpas_get_psk_passphrase(struct wpa_supplicant *wpa_s, u8 *addr,
                  char *buf, size_t buflen)
{
	...
	for (i = 0; i < wpa_s->ap_iface->num_bss; i++) {
		...
		if (os_memcmp(h_ssid->ssid, cur_ssid->ssid, cur_ssid->ssid_len) == 0) {
			for (p = h_ssid->wpa_psk; p; p = p->next) {
				if (os_memcmp(p->addr, addr, ETH_ALEN) == 0) {
					if (cur_ssid->p2p_persistent_group)
					{
                    	os_snprintf(buf, 2*PMK_LEN+1, "%02x%02x%02x%02x%02x%02x%02x%02x"
			                    "%02x%02x%02x%02x%02x%02x%02x%02x"
			                    "%02x%02x%02x%02x%02x%02x%02x%02x"
			                    "%02x%02x%02x%02x%02x%02x%02x%02x",
			                    p->psk[0], p->psk[1], p->psk[2], p->psk[3],
			                    p->psk[4], p->psk[5], p->psk[6], p->psk[7],
			                    p->psk[8], p->psk[9], p->psk[10], p->psk[11],
			                    p->psk[12], p->psk[13], p->psk[14], p->psk[15],
			                    p->psk[16], p->psk[17], p->psk[18], p->psk[19],
			                    p->psk[20], p->psk[21], p->psk[22], p->psk[23],
			                    p->psk[24], p->psk[25], p->psk[26], p->psk[27],
			                    p->psk[28], p->psk[29], p->psk[30], p->psk[31]);
