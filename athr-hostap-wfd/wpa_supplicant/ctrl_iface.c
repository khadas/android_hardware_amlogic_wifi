...
#if 0 /* Display phy info in wpa_cli status */
...
int wps_build_wfa_widi(struct wpa_supplicant *wpa_s, int enable_WIDI, char *dev_name, u8 *dev_type)
{
	...
	if (enable_WIDI == 1)
	{
		...
		strcpy(wpa_s->global->widi_dev_name, dev_name);
					 ...
					 char *cmd)
{
	...
	*value++ = '\0';
	...
	} else if (os_strcasecmp(cmd, "wfd_attach_in_req") == 0) {
		ret = wifi_display_attach_enable(wpa_s->global, !!atoi(value));
	} else if (os_strcasecmp(cmd, "wfd_use_diff_p2p_ie") == 0) {
		ret = wifi_display_diff_p2p_attr(wpa_s->global, !!atoi(value));
					 ...
					 char *buf, size_t buflen)
{
	...
	os_memset(wpa_pairwise, 0x00, sizeof(wpa_pairwise));
	os_memset(psk, 0x00, sizeof(psk));
	os_memset(psk_buf, 0x00, sizeof(psk_buf));
	...
	if (ssid != NULL) {
		...
		if (ssid->wep_tx_keyidx >=0 && ssid->wep_tx_keyidx < 4) {
			...
			if (key_len) {
				...
				os_memset(key, 0x00, sizeof(key));
				os_memset(keyval, 0x00, sizeof(keyval));
...
int wpa_supplicant_phy_sock_deinit(int skfd)
{
	return close(skfd);
}
...
{
	int ret, skfd;
	struct wpa_supplicant_phy_info phy_info = {0};
					    ...
					    char *buf, size_t buflen)
{
	...
	//wpa_printf(MSG_INFO, "%s: %d state=%d", __func__, __LINE__, wpa_s->wpa_state);
#if 0 /* Display phy info in wpa_cli status */
	if (wpa_s->wpa_state >= WPA_ASSOCIATED) {
...
static int widi_p2p_ctrl_peer(struct wpa_supplicant *wpa_s, char *cmd,
			 char *buf, size_t buflen)
{
	...
	res = os_snprintf(pos, end - pos, MACSTR, MAC2STR(info->p2p_device_addr));
...
static int widi_p2p_peers(struct wpa_supplicant *wpa_s, char *buf, size_t buflen)
{
	char addr[32]={0}, cmd[64]={0};
	char *pos, *end;
	int ret=0;


	if (widi_p2p_ctrl_peer(wpa_s, "FIRST", addr, sizeof(addr)) == (-1))
		return -1;
	...
	do {
		ret = os_snprintf(pos, end - pos, "%s,", addr);
		if (ret < 0 || ret >= end - pos)
			return pos - buf;
		pos += ret;
	...
	} while (widi_p2p_ctrl_peer(wpa_s, cmd, addr, sizeof(addr)) != (-1));
	...
	const struct wpa_bss *bss, char *buf, size_t buflen)
{
	...
	{      
		...
		ret = os_snprintf(pos, end - pos, MACSTR "\t%d\t%d\t",
			  MAC2STR(bss->bssid), bss->freq, bss->level-100);
		if (ret < 0 || ret >= end - pos)
			return -1;
		pos += ret;
	...
	struct wpa_supplicant *wpa_s, char *buf, size_t buflen)
{
	...
	dl_list_for_each(bss, &wpa_s->bss_id, struct wpa_bss, list_id) {
        ...
        if (wpa_s->global->widi_dev_found == 1)
        {
			...
			wpa_printf(MSG_DEBUG, "reset widi_dev_found to 0");
            return pos - buf;
        }
#endif /* CONFIG_CUSTOM_WFD */
	}
	...
	char *cmd)
{
	...
	printf( "\nWIDI wpa_supplicant version: 2012-Nov-06\n");
...
void wpa_supplicant_ctrl_iface_scan_results_widi(struct wpa_supplicant *wpa_s)
{
	...
	wpa_printf(MSG_DEBUG, "%s entering...", __func__);
	if (wpa_s->global->widi_enable == 0)
	{
		wpa_printf(MSG_DEBUG, "exit due to no widi packets\n", __func__);
		return;
	}
    
	memset( &local, 0, sizeof(local));

	wpa_supplicant_ctrl_iface_scan_results(wpa_s, buf, sizeof(buf));
	if(buf[0] != '\0')
	...
	if (strncmp(buf, "CUSTOM-WFD-01", 13) == 0)
	{
		...
		wpa_printf(MSG_DEBUG, "	Received L2SD QUERY");
		wpa_printf(MSG_DEBUG, "#############################\n");

			
        if(sendto(wpa_s->global->widi_sock_l2sd, &l2_msg, sizeof(l2_msg), 0,(struct sockaddr*)&local, sizeof(local)) < 0)
        {
            printf("send to L2SD socket fail");
		}
	}
	...
	wpa_printf(MSG_DEBUG, "\n#############################");
	wpa_printf(MSG_DEBUG, "	Received L2TA TRIGGER");
	wpa_printf(MSG_DEBUG, "#############################\n");
			    ...
			    char *buf, size_t buflen)
{
	...
	if (pos2) {
	    ...
	    ssid = wpa_config_get_network(wpa_s->conf, id);
	    if (ssid == NULL || ssid->disabled != 2) {
		    wpa_printf(MSG_DEBUG, "CTRL_IFACE: Could not find id=%d "
			        "for persistent P2P group", id);
		return -1;
	    }
		wpa_printf(MSG_DEBUG, "CTRL_IFACE: Use SSID=%s id=%d "
			    "for persistent P2P group", ssid->ssid, id);
		p2p_persistent_set(wpa_s->global->p2p,  id, ssid->ssid, ssid->ssid_len);
	}
	else
	    p2p_persistent_set(wpa_s->global->p2p,  0, NULL, 0);
...
static int p2p_get_psk_passphrase(struct wpa_supplicant *wpa_s, char *cmd,
                  char *buf, size_t buflen)
{
	struct wpa_ssid *ssid = wpa_s->current_ssid;
	u8 addr[ETH_ALEN];
	...
	return wpas_get_psk_passphrase(wpa_s, &addr, buf, buflen);
}
...
static int p2p_set_passphrase(struct wpa_supplicant *wpa_s, char *passphrase)
{
	struct wpa_ssid *ssid = wpa_s->current_ssid;
	int passphrase_len = os_strlen(passphrase);
	...
	if (passphrase_len < 8 || passphrase_len > 63)
		wpa_printf(MSG_DEBUG, "Passphrase len must be 8~63 characters'");

	wpas_refresh_psk(wpa_s, passphrase);
...
static int p2p_ctrl_invite_auth(struct wpa_supplicant *wpa_s, char *cmd)
{
	...
	if (pos) {
		...
		if (hwaddr_aton(pos, peer))
			return -1;
		return p2p_invite_set_auth(wpa_s->global->p2p, peer, auth);
	}
...
static int p2p_ctrl_group_add(struct wpa_supplicant *wpa_s, char *cmd)
{
	...
	if ((pos = os_strstr(cmd, "persistent="))) 
		return p2p_ctrl_group_add_persistent(wpa_s, pos + 11, freq);
	if (os_strstr(cmd, "persistent") ||
		os_strstr(cmd, "persistent "))
		return wpas_p2p_group_add(wpa_s, 1, freq);
	if (os_strstr(cmd, "freq="))
...
static int p2p_ctrl_set(struct wpa_supplicant *wpa_s, char *cmd)
{
	...
	*param++ = '\0';
	...
	if (os_strcmp(cmd, "go_apsd") == 0) {
		...
		wpas_p2p_set_go_apsd(wpa_s);
		return 0;
	}

	if (os_strcmp(cmd, "tx_sgi") == 0) {
		u32 mask[2];
		u8 threshold;
		char* pos; 

		mask[0] = strtoul(param, NULL, 0);
		pos = os_strchr(param, ',');
		if (pos == NULL)
			return -1;
		pos++;
		
		mask[1] = strtoul(pos, NULL, 0);
		pos = os_strchr(pos, ',');
		if (pos == NULL)
			return -1;
		pos++;
		...
		wpa_drv_set_tx_sgi(wpa_s, mask, threshold);
		return 0;
	}
...
static int p2p_ctrl_info(struct wpa_supplicant *wpa_s, char *buf, size_t buflen)
{
	...
	struct wpa_supplicant *wpa_s_tmp;
	int len_tmp, res;
	char *pos, *end;
	...
	len_tmp = p2p_get_own_info(wpa_s->global->p2p, buf, buflen);
	pos = buf + len_tmp;
	end = buf + buflen;
	for (wpa_s_tmp = global->ifaces; wpa_s_tmp; wpa_s_tmp = wpa_s_tmp->next) {
		if (wpa_s_tmp->p2p_group_interface == P2P_GROUP_INTERFACE_CLIENT) {
			res = os_snprintf(pos, end - pos,
			                  "p2p_if_addr=" MACSTR "\n\n",
			                  MAC2STR(wpa_s_tmp->own_addr));
			if (res < 0 || res >= end - pos)
				return pos - buf;
			pos += res;
		}
...
static int wpa_supplicant_ctrl_iface_kick_sta(
	struct wpa_supplicant *wpa_s, char *cmd)
{
	u8 addr[ETH_ALEN];
	...
	wpa_printf(MSG_DEBUG, "WPA: Kick out station (" MACSTR ")\n", MAC2STR(addr));
	wpas_remove_psk(wpa_s, addr);

	return wpa_drv_kick_sta(wpa_s, addr);
}
					 ...
					 char *buf, size_t *resp_len)
{
	...
	/* To allocate more buferr to store at least 64 APs */
...
#ifdef CONFIG_CUSTOM_WFD
	/* For WiDi L2SD */
	} else if (os_strcmp(buf, "WIDI_SCANMODE_All") == 0) {
	...
	/* For WiDi WFD */
	...
	} else if (os_strncmp(buf, "WIDI_P2P_PEERS", 14) == 0) {
		reply_len = widi_p2p_peers(wpa_s, reply, reply_size);
#endif /* CONFIG_CUSTOM_WFD */
...
#ifdef CE_CUSTOM_0
	} else if (os_strcmp(buf, "WPS_TERMINATE") == 0) {
#else
		...
		if (p2p_ctrl_prov_disc(wpa_s, buf + 14))
			reply_len = -1;
	} else if (os_strncmp(buf, "P2P_GET_PSK_PASSPHRASE ", 23) == 0) {
		reply_len = p2p_get_psk_passphrase(wpa_s, buf + 23, reply, reply_size);
	} else if (os_strcmp(buf, "P2P_GET_PASSPHRASE") == 0) {
		reply_len = p2p_get_passphrase(wpa_s, reply, reply_size);
	} else if (os_strncmp(buf, "P2P_SET_PASSPHRASE ", 19) == 0) {
		if (p2p_set_passphrase(wpa_s, buf + 19))
			reply_len = -1;
		...
		wpas_p2p_dump_rssi(wpa_s);
	} else if (os_strncmp(buf, "P2P_SET_RSSI_THR ", 17) == 0) {
		int rssi_thr = atoi(buf + 17);

		if (rssi_thr > 99) {
			wpa_printf(MSG_ERROR, "The RSSI Range should be 0 ~ 99 ; what you input to be (%d)",
						rssi_thr);
			reply_len = -1;
		} else {
			wpas_p2p_set_rssi_thr(wpa_s, rssi_thr);
		}
#endif /* CONFIG_P2P */
#ifdef CONFIG_WIFI_DISPLAY
		...
		reply_len = wifi_display_subelem_get(wpa_s->global, buf + 16,
						     reply, reply_size);
	} else if (os_strncmp(buf, "WFD_GET_PEER_WFD_IE ", 20) == 0) {
		reply_len = wifi_display_get_peer_wfd_ie(wpa_s->global, buf + 20,
						     reply, reply_size);
	} else if (os_strncmp(buf, "WFD_SET_P2P_DEV_CAPAB ", 22) == 0) {
		if (wifi_display_set_p2p_dev_capab(wpa_s->global, buf + 22) < 0)
			reply_len = -1;
	} else if (os_strncmp(buf, "WFD_GET_P2P_DEV_CAPAB", 21) == 0) {
		reply_len = wifi_display_get_p2p_dev_capab(wpa_s->global, reply, reply_size);
#endif /* CONFIG_WIFI_DISPLAY */
#ifdef CONFIG_INTERWORKING
	...
	} else if (os_strncmp(buf, WPA_CTRL_RSP, os_strlen(WPA_CTRL_RSP)) == 0)
	{
		...
		if (wpa_supplicant_ctrl_iface_sta_autoconnect(wpa_s, buf + 16))
			reply_len = -1;
	} else if (os_strncmp(buf, "KICK_STA ", 9) == 0) {
		if (wpa_supplicant_ctrl_iface_kick_sta(wpa_s, buf + 9))
			reply_len = -1;
