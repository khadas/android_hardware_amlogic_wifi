				  ...
				  struct hostapd_config *conf)
{
	...
	bss->ignore_broadcast_ssid = ssid->ignore_broadcast_ssid = 0;
	...
	if (ssid->wpa_psk_file_path && ssid->wpa_psk_file) {
		bss->ssid.wpa_psk_file_path = os_strdup(ssid->wpa_psk_file_path);
		bss->ssid.wpa_psk_file = os_strdup(ssid->wpa_psk_file);
	}
...
#ifdef CONFIG_P2P
	//[WAR]we need to reset intra bss again for Mck
	wpa_drv_set_intra_bss(wpa_s, wpa_s->conf->p2p_intra_bss);
#endif /* CONFIG_P2P */
