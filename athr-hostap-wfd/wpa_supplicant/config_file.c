...
static void write_wpa_psk_file(FILE *f, struct wpa_ssid *ssid)
{
	char *value;

	if (ssid->wpa_psk_file == NULL)
		return;

	value = wpa_config_get(ssid, "wpa_psk_file");
	if (value == NULL)
		return;
	if (value[0])
		fprintf(f, "\twpa_psk_file=%s\n", value);
	os_free(value);
}
...
static void wpa_config_write_global(FILE *f, struct wpa_config *config)
{
	...
	if (config->p2p_max_clients)
		fprintf(f, "p2p_max_clients=%u\n", config->p2p_max_clients);
	if (config->p2p_scan_band)
		fprintf(f, "p2p_scan_band=%u\n", config->p2p_scan_band);
	if (config->p2p_expire_age)
		fprintf(f, "p2p_expire_age=%u\n", config->p2p_expire_age);
	if (config->p2p_expire_poll)
		fprintf(f, "p2p_expire_poll=%u\n", config->p2p_expire_poll);
	if (config->wpa_psk_file)
		fprintf(f, "wpa_psk_file=%s\n", config->wpa_psk_file);
#endif /* CONFIG_P2P */
...
int wpa_config_write(const char *name, struct wpa_config *config)
{
	...
	for (ssid = config->ssid; ssid; ssid = ssid->next) {
		...
		if ((ssid->mode == WPAS_MODE_P2P_GO) &&
		    (config->wpa_psk_file) && (ssid->wpa_psk_file == NULL)) {
			f_name = wpa_psk_file_name_get(config->wpa_psk_file, ssid->ssid);
			if (f_name == NULL)
				return -1;
			ssid->wpa_psk_file = f_name;
		}
