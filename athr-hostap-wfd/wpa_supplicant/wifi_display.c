...
static int wifi_display_update_wfd_ie(struct wpa_global *global)
{
	...
	if (!global->wifi_display) {
		...
		p2p_set_wfd_ie_invitation_req(global->p2p, NULL);
		p2p_set_wfd_ie_invitation_resp(global->p2p, NULL);
		p2p_set_wfd_ie_prov_disc_req(global->p2p, NULL);
		p2p_set_wfd_ie_prov_disc_resp(global->p2p, NULL);
		p2p_set_wfd_ie_go_neg_req(global->p2p, NULL);
	...
	if (!global->wifi_display_attach) {
		...
		ie = wifi_display_encaps(buf);
		wpa_hexdump_buf(MSG_DEBUG, "WFD: WFD IE for GO Negotiation Request", ie);
		p2p_set_wfd_ie_go_neg_req(global->p2p, ie);
	...
	if (!global->wifi_display_attach) {
		...
		ie = wifi_display_encaps(buf);
		wpa_hexdump_buf(MSG_DEBUG, "WFD: WFD IE for P2P Invitation "
				"Request", ie);
		p2p_set_wfd_ie_invitation_req(global->p2p, ie);
	}

	ie = wifi_display_encaps(buf);
	wpa_hexdump_buf(MSG_DEBUG, "WFD: WFD IE for P2P Invitation "
			"Response", ie);
	p2p_set_wfd_ie_invitation_resp(global->p2p, ie);
...
void wifi_display_enable(struct wpa_global *global, int enabled)
{
	...
	p2p_wifi_display_set(global->p2p, enabled);
	wifi_display_update_wfd_ie(global);
}


int wifi_display_attach_enable(struct wpa_global *global, int enabled)
{
	if (global->wifi_display) {
		wpa_printf(MSG_DEBUG, "WFD: Wi-Fi Display IE Attach of Request Frames is %s",
		           enabled ? "enabled" : "disabled");
		printf("WFD: Wi-Fi Display IE Attach of Request Frames is %s\n",
		       enabled ? "enabled" : "disabled");
		global->wifi_display_attach = enabled;
		wifi_display_update_wfd_ie(global);
		return 0;
	} else {
		wpa_printf(MSG_DEBUG, "WFD: Wi-Fi Display is disabled. "
		           "Please enable wifi_display in advance.");
		printf("WFD: Wi-Fi Display is disabled. "
		       "Please enable wifi_display in advance.\n");
		return -1;
	}
}
...
int wifi_display_diff_p2p_attr(struct wpa_global *global, int enabled)
{
	if (global->wifi_display) {
		wpa_printf(MSG_DEBUG, "WFD: Wi-Fi Display Over P2P with different P2P-IE is %s",
		           enabled ? "enabled" : "disabled");
		printf("WFD: Wi-Fi Display Over P2P with different P2P-IE is %s\n",
		       enabled ? "enabled" : "disabled");
		global->wifi_display_diff_p2p_attr = enabled;
		p2p_wfd_set_diff_attr(global->p2p, enabled);
		return 0;
	} else {
		wpa_printf(MSG_DEBUG, "WFD: Wi-Fi Display is disabled. "
		           "Please enable wifi_display in advance.");
...
int wifi_display_get_peer_wfd_ie(struct wpa_global *global, char *cmd,
			     char *buf, size_t buflen)
{
	u8 addr[ETH_ALEN], frame_type = 0;

	if (hwaddr_aton(cmd, addr) < 0)
		return -1;

	frame_type = atoi(cmd + 18);
	if (frame_type <= 0 || frame_type >= MAX_FRAME_TYPE)
		return -1;
	wpa_printf(MSG_DEBUG, "WFD: Peer = " MACSTR ", Frame-Type = %d", MAC2STR(addr), frame_type);

	return p2p_get_peer_wfd_ie(global->p2p, addr, frame_type, buf);
}


int wifi_display_set_p2p_dev_capab(struct wpa_global *global, char *cmd)
{
	char *pos;
	int bitmap;
	int enabled;
	...
	*pos++ = '\0';
	bitmap = atoi(cmd);
	enabled = atoi(pos);
	if (bitmap < 0 || bitmap >= 6 || enabled < 0 || enabled > 1)
		return -1;

	wpa_printf(MSG_DEBUG, "WFD: bitmap = %d, enabled = %d", bitmap, enabled);
	p2p_wfd_set_dev_capab(global->p2p, bitmap, enabled);

	return 0;
}


int wifi_display_get_p2p_dev_capab(struct wpa_global *global,
			     char *buf, size_t buflen)
{
	return p2p_wfd_get_dev_capab(global->p2p, buf);
}