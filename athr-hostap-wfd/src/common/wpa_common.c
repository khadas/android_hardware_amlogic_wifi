...
int wpa_psk_file_update(char *wpa_psk_file, const u8 *peer, u8* psk)
{
	FILE *f = fopen(wpa_psk_file, "r");
	char buf[8192], tmp_buf[128];
	char *pos, *end;
	int line = 0, ret = 0, res;
	u8 addr[ETH_ALEN];

	if (f == NULL) {
		wpa_printf(MSG_DEBUG, "Failed to open '%s' for reading", wpa_psk_file);
		return -1;
	} else {
		os_memset(buf, 0, sizeof(buf));
		pos = buf;
		end = buf + sizeof(buf);
		while (fgets(tmp_buf, sizeof(tmp_buf), f)) {
			line++;
			...
			if (hwaddr_aton(tmp_buf, addr)) {
				wpa_printf(MSG_ERROR, "Invalid MAC address '%s' on "
				           "line %d in '%s'", tmp_buf, line, wpa_psk_file);
				ret = -1;
				break;
			}

			if (os_memcmp(addr, peer, ETH_ALEN) == 0)
				continue;

			res = os_snprintf(pos, end - pos, "%s", tmp_buf);
	...
	if (ret) {
		wpa_printf(MSG_DEBUG, "Failed to remove psk from '%s'", wpa_psk_file);
		return -1;
	} else {
		f = fopen(wpa_psk_file, "w");
		if (f == NULL) {
			wpa_printf(MSG_DEBUG, "Failed to open '%s' for writing", wpa_psk_file);
			return -1;
		}
