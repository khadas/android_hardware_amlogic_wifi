...
static u8 * hostapd_gen_probe_resp(struct hostapd_data *hapd,
                                   ...
                                   int is_p2p, int is_wfd, size_t *resp_len)
{
	...
	*pos++ = hapd->conf->ssid.ssid_len;
	...
	if ((hapd->conf->p2p & P2P_ENABLED) && is_p2p &&
		...
		if (is_wfd) {
			...
			u8 pure_p2p_ie[512], *pos_current;
			u16 pure_p2p_ie_len, i;

			pos_current = wpabuf_head(hapd->p2p_probe_resp_ie);
			os_memset(pure_p2p_ie, 0, 512);
			pure_p2p_ie_len = 0;
			while (pos_current && (*pos_current == 0xDD)) {
				i = *(pos_current + 1);
				if ((i > 2) && (*(pos_current + 5) == 0x09)) {
					os_memcpy(pure_p2p_ie, pos_current, i + 2);
					pure_p2p_ie_len += (i + 2);
				}
				pos_current += (i + 2);
			}
			...
			if (pure_p2p_ie_len) {
				os_memcpy(pos, pure_p2p_ie, pure_p2p_ie_len);
				pos += pure_p2p_ie_len;
			}
		}
	}
...
static int handle_probe_req_search_device_addr(struct hostapd_data *hapd, struct sta_info *sta,
			      void *ctx) 
{
	const u8 *dev_addr = NULL;
	const u8 *query = (u8 *)ctx;

#ifdef CONFIG_P2P
	dev_addr = p2p_group_get_dev_addr(hapd->p2p_group, sta->addr);

	if ((dev_addr) &&
	    (os_memcmp(dev_addr, query, ETH_ALEN) == 0))
		return 1;
#endif /* CONFIG_P2P */
                      ...
                      int ssi_signal)
{
	...
	if (hapd->p2p) {
		...
		if (p2p && p2p_parse_p2p_ie(p2p, &msg)) {
			wpa_printf(MSG_MSGDUMP, "P2P: Ignore Probe Request "
				   "with incorrect P2P IE");
			wpabuf_free(p2p);
			return;
		}
		if (msg.device_id) {
			if (os_memcmp(msg.device_id, hapd->p2p->cfg->dev_addr, ETH_ALEN) == 0) {
				wpa_printf(MSG_DEBUG, "P2P: Probe Request with "
							"directed Device ID");
			} else if (ap_for_each_sta(hapd, handle_probe_req_search_device_addr, 
							(void *)msg.device_id)) {
				wpa_printf(MSG_DEBUG, "P2P: Probe Request with "
							"P2P-client's Device ID");
			} else {
				wpa_printf(MSG_DEBUG, "P2P: Ignore Probe Request "
					   "with incorrect P2P device id");
				wpabuf_free(p2p);
				return;
			}
		}
	...
	if (hapd->p2p && elems.wps_ie) {
		...
		if (wps && wps_parse_msg(wps, &attr)) {
			wpa_printf(MSG_MSGDUMP, "P2P: Ignore Probe Request "
				   "with incorrect WPS IE");
			wpabuf_free(wps);
			return;
		}
...
#ifdef CONFIG_P2P//saber328,FY11
	if(hapd->conf->p2p & P2P_GROUP_OWNER) {
		if ((elems.ssid == NULL) && elems.ssid_len == 0) {
			/* No SSID found. Do not handle this here. */
			return;
		}
		if ((elems.supp_rates_len == 0) && (elems.ext_supp_rates_len == 0)) {
	...
	resp = hostapd_gen_probe_resp(hapd, sta, mgmt, elems.p2p != NULL, elems.wfd != NULL,
	                              &resp_len);
	if (resp == NULL)
		return;
