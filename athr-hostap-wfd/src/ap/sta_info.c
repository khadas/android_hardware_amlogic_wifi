			   ...
			   int authorized)
{
	...
	if (authorized) {
		...
		if (dev_addr) {
			...
			ret = p2p_get_peer_wfd_ie(hapd->p2p, dev_addr, 0, wfd_subelems);
			if (ret > 0)
				wpa_msg(hapd->msg_ctx_parent, MSG_INFO, "%s", wfd_subelems);
		}
#endif /* CONFIG_P2P */		
	} else {
		if (dev_addr)
...
{
	if (sta == NULL && reason == WLAN_REASON_PREV_AUTH_NOT_VALID)
		wpa_msg(hapd->msg_ctx, MSG_INFO, AP_STA_DISCONNECTED MACSTR, MAC2STR(addr));
