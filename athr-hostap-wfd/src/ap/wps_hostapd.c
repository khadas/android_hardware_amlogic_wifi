...
static void hostapd_psk_existence_check(struct hostapd_data *hapd, const u8 *mac_addr)
{
	struct hostapd_ssid *ssid = &hapd->conf->ssid;
	struct hostapd_wpa_psk *p, *current_p = NULL;

	for (p = ssid->wpa_psk; p; p = p->next) {
		if (os_memcmp(mac_addr, p->addr, ETH_ALEN) == 0) {
			if (current_p == NULL)
				ssid->wpa_psk = p->next;
			else
			...
			if (ssid->wpa_psk_file) {
				...
				if (ssid->wpa_psk_file_path)
					f_name = wpa_psk_file_path_name_get(ssid->wpa_psk_file_path,
					                                    ssid->wpa_psk_file,
					                                    ssid->ssid);
				if (f_name) {
					wpa_psk_file_update(f_name, mac_addr, NULL);
					os_free(f_name);
				} else
					wpa_printf(MSG_DEBUG, "Can't get full wpa_psk_file path/name");
			}
			break;
		}
		current_p = p;
				  ...
				  size_t psk_len)
{
	...
	/* Check the existence of PSK for the new join STA */
	hostapd_psk_existence_check(hapd, mac_addr);
