...
SM_STATE(WPA_PTK, INITPSK)
{
	...
	if (!is_zero_ether_addr(sm->p2p_dev_addr))
		sta_addr = sm->p2p_dev_addr;

	psk = wpa_auth_get_psk(sm->wpa_auth, sta_addr, NULL);
	if (psk) {
...
SM_STATE(WPA_PTK, PTKCALCNEGOTIATING)
{
	...
	for (;;) {
		if (wpa_key_mgmt_wpa_psk(sm->wpa_key_mgmt)) {
			...
			pmk = wpa_auth_get_psk(sm->wpa_auth, sta_addr, pmk);
			if (pmk == NULL)
				break;
		} else
			pmk = sm->PMK;
...
SM_STEP(WPA_PTK)
{
	...
	case WPA_PTK_INITPSK:
		...
		if (wpa_auth_get_psk(sm->wpa_auth, sta_addr, NULL))
			SM_ENTER(WPA_PTK, PTKSTART);
		else {
			wpa_auth_logger(sm->wpa_auth, sm->addr, LOGGER_INFO,
...
void wpa_sm_sta_set_p2p_dev_addr(struct wpa_state_machine *sm, const u8 *addr)
{
	if (sm)
		os_memcpy(sm->p2p_dev_addr, addr, ETH_ALEN);
}
