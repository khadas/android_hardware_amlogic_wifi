			...
			const u8 *req_ies, size_t req_ies_len, int reassoc)
{
	...
	if (elems.p2p) {
		...
		sta->p2p_ie = ieee802_11_vendor_ie_concat(req_ies, req_ies_len,
							  P2P_IE_VENDOR_TYPE);
		if (elems.wfd) {
			struct wpabuf *wfd = ieee802_11_vendor_ie_concat(req_ies, req_ies_len, WFD_IE_VENDOR_TYPE);
			int ret = p2p_wfd_device_type_check(hapd->p2p, wfd);

			wpabuf_free(wfd);
			if (ret) {
				wpa_printf(MSG_DEBUG, "WFD-IE Device Type Mismatched");
				hostapd_drv_sta_kick(hapd, sta->addr);
...
skip_wpa_check:
	...
	if (sta->p2p_ie && hapd->p2p_group) {
		u8 *p2p_dev_addr = p2p_group_get_member_dev_addr(hapd->p2p_group, sta->addr);

		if (p2p_dev_addr) {
			os_memcpy(sta->p2p_dev_addr, p2p_dev_addr, ETH_ALEN);
			if (elems.wfd) {
				p2p_set_peer_wfd_ie(hapd->p2p, p2p_dev_addr,
					WFD_IE_FROM_ASSOC_REQ, req_ies, req_ies_len);
				wpa_printf(MSG_DEBUG, "Save WFD-IE from Assoc-Req/Re-Assoc-Req frame");
			} else{
				p2p_set_peer_wfd_ie(hapd->p2p, p2p_dev_addr,
					WFD_IE_FROM_ASSOC_REQ, NULL, 0);
			}
		}
	}
#endif /* CONFIG_P2P */
