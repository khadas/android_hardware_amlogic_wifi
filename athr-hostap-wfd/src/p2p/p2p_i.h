...
#define P2P_DEV_RSSI_OVER_THR(_p2p, _dev) (((_dev->probe_rssi/4) >= _p2p->p2p_rssi_thr) ? 1 : 0 )
...
#define P2P_DEV_INVITATION_AUTHORIZED BIT(18)
	unsigned int flags;
...
struct p2p_data {
	...
	int cross_connect;
	int group_limit_reached;

	int best_freq_24;
	int best_freq_5;
	int best_freq_overall;

	/* FIX : force_freq may use structure instead of integer to pack 
		more information. */
	int acs_state;
	...
	struct wpabuf *wfd_ie_invitation_req;
	struct wpabuf *wfd_ie_invitation_resp;
	struct wpabuf *wfd_ie_prov_disc_req;
	struct wpabuf *wfd_ie_prov_disc_resp;
	struct wpabuf *wfd_ie_go_neg_req;
	...
	u8 p2p_wifi_display;
	u8 p2p_diff_attr_wfd;
	u8 p2p_dev_capab_wfd;
...
#define P2P_ADD_DEVICE_FROM_SCAN_RESULT     1
#define P2P_ADD_DEVICE_FROM_INVITATION      2
#define P2P_ADD_DEVICE_FROM_PROVISION       3
