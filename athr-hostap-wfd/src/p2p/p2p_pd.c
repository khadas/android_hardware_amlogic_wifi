...
#ifdef CONFIG_WIFI_DISPLAY
	if (p2p->wfd_ie_prov_disc_req && wfd_subelems)
...
#ifdef CE_CUSTOM_0 /* only support PBC */
	if (!(msg.wps_config_methods & WPS_CONFIG_PUSHBUTTON)) {
		wpa_msg(p2p->cfg->msg_ctx, MSG_INFO, "P2P: Unsupported "
...
out:
	if (((p2p->cfg->config_methods == WPS_CONFIG_PUSHBUTTON) &&
		 (msg.wps_config_methods != WPS_CONFIG_PUSHBUTTON)) ||
		(((p2p->cfg->config_methods & WPS_CONFIG_KEYPAD) == 0) &&
		 (msg.wps_config_methods == WPS_CONFIG_KEYPAD)) ||
		(((p2p->cfg->config_methods & WPS_CONFIG_DISPLAY) == 0) &&
		 (msg.wps_config_methods == WPS_CONFIG_DISPLAY)))
		resp = p2p_build_prov_disc_resp(p2p, msg.dialog_token, 0,
						msg.group_id, msg.group_id_len, msg.wfd_subelems);
	else
	...
	p2p->pending_action_state = P2P_PENDING_PROV_DISC_REQ;
				...
				const u8 *data, size_t len)
{
	...
	if (msg.wfd_subelems) {
		...
		dev->info.wfd_subelems = wpabuf_dup(msg.wfd_subelems);
		dev->info.wfd_subelems_come_from[0] = WLAN_FC_STYPE_ACTION;
		dev->info.wfd_subelems_come_from[1] = P2P_PROV_DISC_RESP;
		wpabuf_free(dev->info.wfd_subelems_from[WFD_IE_FROM_PROV_RESP]);
		dev->info.wfd_subelems_from[WFD_IE_FROM_PROV_RESP] = wpabuf_dup(msg.wfd_subelems);
		wpa_msg(p2p->cfg->msg_ctx, MSG_DEBUG, "P2P: Save WFD-IE from "
		      ...
		      u16 config_methods, int join, int force_freq)
{
	...
	peer_info = p2p_wfd_get_peer_info(p2p, peer_addr);
	if (peer_info == NULL) {
		wpa_msg(p2p->cfg->msg_ctx, MSG_INFO, "P2P: Peer is not Found");
		return -1;
	} else {
		if (p2p_wfd_device_type_check(p2p, peer_info->wfd_subelems)) {
			wpa_msg(p2p->cfg->msg_ctx, MSG_INFO, "WFD-IE Device Type Mismatched");
			return 0;
		}
	}
...
{	
	return (p2p->pending_action_state == P2P_PENDING_PROV_DISC_REQ);
}
