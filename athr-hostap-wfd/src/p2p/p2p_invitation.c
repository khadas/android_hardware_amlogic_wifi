						...
						const u8 *go_dev_addr)
{
	...
	struct wpabuf *wfd_ie = p2p->wfd_ie_invitation_req;
...
#ifdef CONFIG_WIFI_DISPLAY
	if (wfd_ie && peer->info.wfd_subelems)
...
#ifdef CONFIG_WIFI_DISPLAY
	struct wpabuf *wfd_ie = p2p->wfd_ie_invitation_resp;
...
#ifdef CONFIG_WIFI_DISPLAY
	if (wfd_ie && peer->info.wfd_subelems_from[WFD_IE_FROM_INVI_REQ])
		wpabuf_put_buf(buf, wfd_ie);
#endif /* CONFIG_WIFI_DISPLAY */
				...
				const u8 *data, size_t len, int rx_freq)
{
	...
	if (p2p->pending_action_state == P2P_PENDING_INVITATION_RESPONSE)
		return;
	...
	if (dev == NULL || (dev->flags & P2P_DEV_PROBE_REQ_ONLY)) {
		...
		if (p2p_add_device(p2p, sa, rx_freq, 0, data + 1, len - 1, 0,
		                   P2P_ADD_DEVICE_FROM_INVITATION)) {
			wpa_msg(p2p->cfg->msg_ctx, MSG_DEBUG,
	...
	if (p2p->cfg->invitation_process) {
		if ((p2p->cfg->persistent_reconnect == 1) ||
		    (dev->flags & P2P_DEV_INVITATION_AUTHORIZED))
			auth = 1;
		status = p2p->cfg->invitation_process(
			p2p->cfg->cb_ctx, sa, msg.group_bssid, msg.group_id,
			msg.group_id + ETH_ALEN, msg.group_id_len - ETH_ALEN,
			&go, group_bssid, &op_freq, persistent, auth);
	...
	if (op_freq) {
		...
		/* no operation channel configuration, check which channel is in intersect. */
		if (!p2p->cfg->cfg_op_channel) {
			...
			if (p2p->cfg->p2p_scan_band == 1) {
				best_freq[0] = p2p->best_freq_24;
				best_freq[1] = p2p->best_freq_5;
				best_freq[2] = p2p->best_freq_overall;
			} else if (p2p->cfg->p2p_scan_band == 2) {
				best_freq[0] = p2p->best_freq_5;
				best_freq[1] = p2p->best_freq_24;
				best_freq[2] = p2p->best_freq_overall;
			} else {
				best_freq[0] = p2p->best_freq_overall;
				best_freq[1] = p2p->best_freq_5;
				best_freq[2] = p2p->best_freq_24;
			}
			p2p_channels_intersect(&p2p->cfg->channels, &dev->channels,
			                       &intersection);
			while (i < 3) {
				if (best_freq[i] > 0 &&
					p2p_supported_freq(p2p, best_freq[i]) &&
            ...
            /* can't find best freq, use peer wanted if exist. */
			if (i == 3) {
				if (msg.operating_channel) {
					op_freq = p2p_channel_to_freq((const char *)
					...
					if (op_freq < 0) {
						wpa_msg(p2p->cfg->msg_ctx, MSG_DEBUG,
							...
							msg.operating_channel[0], msg.operating_channel[1],
							msg.operating_channel[3], msg.operating_channel[4]);
						status = P2P_SC_FAIL_NO_COMMON_CHANNELS;
						goto fail;
					}
					if (status == P2P_SC_SUCCESS) {
						reg_class = msg.operating_channel[3];
						channel = msg.operating_channel[4];
						channels = &intersection;
					}
				} else {
					int j, i, peer_prefer_op_freq;
					for(j = 0; j < intersection.reg_classes; j++) {
						for(i = 0; i < intersection.reg_class[j].channels;i++) {
							peer_prefer_op_freq = p2p_channel_to_freq(p2p->cfg->country,
																		(int)intersection.reg_class[j].reg_class,
																		(int)intersection.reg_class[j].channel[i]);
							if (peer_prefer_op_freq > 0)
								break;
						}
					}
					if (status == P2P_SC_SUCCESS) {
			...
			if (!p2p_channels_includes(&intersection, reg_class, channel))
			{
				wpa_printf(MSG_INFO, "P2P: forced freq %d MHz not in the supported "
					"channels interaction, reg_class=%d, channel=%d\n\r", op_freq,reg_class,channel);
				status = P2P_SC_FAIL_NO_COMMON_CHANNELS;
				goto fail;
			}
		} else {
		...
		if (status == P2P_SC_SUCCESS) {
			if (channels && p2p->cfg->p2p_scan_band) {
				struct p2p_channels res;
				p2p_channels_skip(channels,
								&res, p2p->cfg->p2p_scan_band);
				os_memcpy(channels, &res,
					  sizeof(struct p2p_channels));
			}
		}        
		wpa_printf(MSG_INFO, "P2P: GO operating channel preference: %d MHz\n\r", 
							op_freq);
	}
...
fail:
	...
	if (msg.wfd_subelems) {
		...
		if (status == P2P_SC_SUCCESS) {
			if (p2p_wfd_device_type_check(p2p, msg.wfd_subelems)) {
				wpa_msg(p2p->cfg->msg_ctx, MSG_DEBUG, "WFD-IE Device Type Mismatched");
...
#ifdef CONFIG_CUSTOM_WFD
	{
		...
		wpa_msg(wpa_s, MSG_INFO, 
			"P2P: Received Invitation Request from " MACSTR ", send Invitation Response and wait for ACK", MAC2STR(sa));
	}
				 ...
				 const u8 *data, size_t len)
{
	...
	if (*msg.status == P2P_SC_SUCCESS) { //check channel list
		struct p2p_channels intersection;
		u8 reg_class = 0, channel = 0;
		int peer_prefer_op_freq;
		int prefer_op_freq[3];
		int i = 0;

		prefer_op_freq[0] = p2p->inv_op_freq;
		prefer_op_freq[1] = 0;
		prefer_op_freq[2] = 0;

		if (p2p_peer_channels_check(p2p, &p2p->cfg->channels, dev,
									msg.channel_list, msg.channel_list_len) < 0) {
			wpa_printf(MSG_INFO, "P2P: no common channel found...");		
			p2p_parse_free(&msg);
			return;
		}
		...
		if (msg.operating_channel) {
			peer_prefer_op_freq = p2p_channel_to_freq((const char *)
														msg.operating_channel,
														msg.operating_channel[3],
														msg.operating_channel[4]);
			prefer_op_freq[1] = peer_prefer_op_freq;
			if ((prefer_op_freq[0] == prefer_op_freq[1]) && (peer_prefer_op_freq != -1))
				goto result_process;
		}

		while(1) {
			if (prefer_op_freq[i] == 0) {
				int j, i;
				for(j = 0; j < intersection.reg_classes; j++) {
					for(i = 0; i < intersection.reg_class[j].channels;i++) {
						peer_prefer_op_freq = p2p_channel_to_freq(p2p->cfg->country,
																	...
																	(int)intersection.reg_class[j].channel[i]);
						if (peer_prefer_op_freq > 0) {
							p2p->inv_op_freq = peer_prefer_op_freq;
							break;
						}
					}
					if (peer_prefer_op_freq > 0)
			...
			if (p2p_freq_to_channel(p2p->cfg->country, prefer_op_freq[i],
									&reg_class, &channel) < 0) {
				i++;
				continue;
			}
			...
			p2p->inv_op_freq = prefer_op_freq[i];
			break;
		}
	}
...
result_process:
	if (p2p->cfg->invitation_result)
		p2p->cfg->invitation_result(p2p->cfg->cb_ctx, *msg.status,
									msg.group_bssid, &p2p->inv_op_freq);

	if (p2p->inv_op_freq != 0)
		dev->oper_freq = p2p->inv_op_freq;

	if (msg.wfd_subelems) {
		wpabuf_free(dev->info.wfd_subelems);
		dev->info.wfd_subelems = wpabuf_dup(msg.wfd_subelems);
		dev->info.wfd_subelems_come_from[0] = WLAN_FC_STYPE_ACTION;
		dev->info.wfd_subelems_come_from[1] = P2P_INVITATION_RESP;
		wpabuf_free(dev->info.wfd_subelems_from[WFD_IE_FROM_INVI_RESP]);
		dev->info.wfd_subelems_from[WFD_IE_FROM_INVI_RESP] = wpabuf_dup(msg.wfd_subelems);
		wpa_msg(p2p->cfg->msg_ctx, MSG_DEBUG, "P2P: Save WFD-IE from "
	       ...
	       int persistent_group)
{
	...
	if (force_freq) {
		...
		/* no operation channel configuration, use ACS to find out. */
		if (!p2p->cfg->cfg_op_channel) { 
			int best_freq[3];
			int i = 0;
			int op_freq;
			...
			/* use default if no ACS report or report is not accept in P2P. */
			if (i == 3) {
			...
			p2p->op_reg_class = p2p->cfg->op_reg_class;
			p2p->op_channel = p2p->cfg->op_channel;
			p2p->inv_op_freq = p2p_channel_to_freq(p2p->cfg->country,
			                                       p2p->op_reg_class,
			                                       p2p->op_channel);
		}
	...
	if (p2p->cfg->persistent_reconnect == 0)
		dev->flags |= P2P_DEV_INVITATION_AUTHORIZED;
	return p2p_invite_send(p2p, dev, go_dev_addr);
}


int p2p_invite_set_auth(struct p2p_data *p2p, const u8 *peer, int auth)
{
	struct p2p_device *dev;

	dev = p2p_get_device(p2p, peer);
	if (dev == NULL) {
		wpa_msg(p2p->cfg->msg_ctx, MSG_DEBUG, "P2P: Device not found for authorizing invitation");
		return -1;
	}
