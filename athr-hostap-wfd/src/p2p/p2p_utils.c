...
void p2p_channels_skip(const struct p2p_channels *a,
                          struct p2p_channels *res, int band_inc)
{
	size_t i;
	...
	for (i = 0; i < a->reg_classes; i++) {
		...
		switch(a_reg->reg_class) {
			case 81: /* Operating class 81 - 2.4 GHz band channels 1..13 */
			case 82: /* Operating class 81 - 2.4 GHz band channels 14 */
				if (band_inc == 1) {
					os_memcpy(&res->reg_class[res->reg_classes], a_reg, sizeof(struct p2p_reg_class));
					res->reg_classes++;
				}
				break;

			case 115: /* Operating class 115 - 5 GHz, channels 36-48 */
			case 124: /* Operating class 124 - 5 GHz, channels 149-161 */
			case 116: /* Operating class 115 - 5 GHz, channels 36-44 */
			case 117: /* Operating class 115 - 5 GHz, channels 40-48 */
			case 126: /* Operating class 115 - 5 GHz, channels 149-157 */
			case 127: /* Operating class 115 - 5 GHz, channels 153-161 */
				if (band_inc == 1) {
					os_memcpy(&res->reg_class[res->reg_classes], a_reg, sizeof(struct p2p_reg_class));
					res->reg_classes++;
				}
