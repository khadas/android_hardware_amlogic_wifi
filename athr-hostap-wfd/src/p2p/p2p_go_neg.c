					    ...
					    struct p2p_device *peer)
{
	...
	if (p2p->wfd_ie_go_neg_req)
		extra = wpabuf_len(p2p->wfd_ie_go_neg_req);
...
#ifdef CONFIG_WIFI_DISPLAY
	if (p2p->wfd_ie_go_neg_req && peer->info.wfd_subelems)
		wpabuf_put_buf(buf, p2p->wfd_ie_go_neg_req);
#endif /* CONFIG_WIFI_DISPLAY */
...
#ifdef CONFIG_WIFI_DISPLAY
	if (p2p->wfd_ie_go_neg && peer && peer->info.wfd_subelems_from[WFD_IE_FROM_GO_NEG_REQ])
			    ...
			    const u8 *data, size_t len, int rx_freq)
{
	...
	if (dev && dev->flags & P2P_DEV_USER_REJECTED) {
		...
		if (go) {
	    	...
	    	{
				if(p2p->persistent_ssid_len != 0) {
					os_memcpy(p2p->ssid, p2p->persistent_ssid, p2p->persistent_ssid_len);
					p2p->ssid_len = p2p->persistent_ssid_len;
				}
		    }
		}
	...
	if (msg.wfd_subelems) {
		...
		if (dev && dev->info.wfd_subelems_from[WFD_IE_FROM_GO_NEG_REQ]) {
			...
			pos += wpa_snprintf_hex(pos, end - pos,
						wpabuf_head(dev->info.wfd_subelems_from[WFD_IE_FROM_GO_NEG_REQ]),
						wpabuf_len(dev->info.wfd_subelems_from[WFD_IE_FROM_GO_NEG_REQ]));
			...
			pos += res;
			wpa_msg(p2p->cfg->msg_ctx, MSG_INFO, "%s", wfd_subelems);
		}
					     ...
					     const u8 *resp_chan, int go)
{
	...
	//+FLUG20120524
		if(peer->go_state == LOCAL_GO)//Local prefer to be a P2P GO
...
#ifdef CONFIG_WIFI_DISPLAY
	if (p2p->wfd_ie_go_neg && peer->info.wfd_subelems_from[WFD_IE_FROM_GO_NEG_RESP])
		wpabuf_put_buf(buf, p2p->wfd_ie_go_neg);
#endif /* CONFIG_WIFI_DISPLAY */
			     ...
			     const u8 *data, size_t len, int rx_freq)
{
	...
	if (msg.wfd_subelems) {
		...
		dev->info.wfd_subelems = wpabuf_dup(msg.wfd_subelems);
		dev->info.wfd_subelems_come_from[0] = WLAN_FC_STYPE_ACTION;
		dev->info.wfd_subelems_come_from[1] = P2P_GO_NEG_RESP;
		wpabuf_free(dev->info.wfd_subelems_from[WFD_IE_FROM_GO_NEG_RESP]);
		dev->info.wfd_subelems_from[WFD_IE_FROM_GO_NEG_RESP] = wpabuf_dup(msg.wfd_subelems);
		wpabuf_free(dev->info.wfd_subelems_from[0]);
		dev->info.wfd_subelems_from[0] = wpabuf_dup(msg.wfd_subelems);
		wpa_msg(p2p->cfg->msg_ctx, MSG_DEBUG, "P2P: Save WFD-IE from "
		...
		if (status == P2P_SC_SUCCESS) {
			if (p2p_wfd_device_type_check(p2p, msg.wfd_subelems)) {
				status = P2P_SC_FAIL_INCOMPATIBLE_PARAMS;
				wpa_msg(p2p->cfg->msg_ctx, MSG_INFO,
				        "P2P-GO-NEG-RESPONSE WFD-IE Device Type Mismatched");
			}
		}
	}
			     ...
			     const u8 *data, size_t len)
{
	...
	if (msg.wfd_subelems) {
		...
		dev->info.wfd_subelems = wpabuf_dup(msg.wfd_subelems);
		dev->info.wfd_subelems_come_from[0] = WLAN_FC_STYPE_ACTION;
		dev->info.wfd_subelems_come_from[1] = P2P_GO_NEG_CONF;
		wpabuf_free(dev->info.wfd_subelems_from[WFD_IE_FROM_GO_NEG_CONF]);
		dev->info.wfd_subelems_from[WFD_IE_FROM_GO_NEG_CONF] = wpabuf_dup(msg.wfd_subelems);
		wpabuf_free(dev->info.wfd_subelems_from[0]);
		dev->info.wfd_subelems_from[0] = wpabuf_dup(msg.wfd_subelems);
		wpa_msg(p2p->cfg->msg_ctx, MSG_DEBUG, "P2P: Save WFD-IE from "
