...
struct p2p_peer_info {
	...
	u8 wfd_subelems_come_from[2];

#define WFD_SUBELEMS_LEN            1024
#define WFD_IE_FROM_BEACON          1
#define WFD_IE_FROM_PROBE_REQ       2
#define WFD_IE_FROM_PROBE_RESP      3
#define WFD_IE_FROM_ASSOC_REQ       4
#define WFD_IE_FROM_ASSOC_RESP      5
#define WFD_IE_FROM_GO_NEG_REQ      6
#define WFD_IE_FROM_GO_NEG_RESP     7
#define WFD_IE_FROM_GO_NEG_CONF     8
#define WFD_IE_FROM_INVI_REQ        9
#define WFD_IE_FROM_INVI_RESP       10
#define WFD_IE_FROM_PROV_REQ        11
#define WFD_IE_FROM_PROV_RESP       12
#define MAX_FRAME_TYPE              (WFD_IE_FROM_PROV_RESP + 1)

	struct wpabuf *wfd_subelems_from[MAX_FRAME_TYPE];
};
...
#define P2P_SEC_DEVICE_TYPES 5
	...
	/**
	 * p2p_scan_band - Only scan channels by band
	 */ 
	int p2p_scan_band;

	/**
	 * p2p_expire_age - p2p entry age after which it can be expired
	 *
	 * This value controls the time in seconds after which a p2p entry
	 * gets removed if it has not been updated or is not in use.
	 */
	unsigned int p2p_expire_age;

	/**
	 * p2p_expire_poll - internal to check p2p entry age to be expired
	 *
	 * This value controls the internal in seconds to check which a p2p entry
	 * gets removed if it has not been updated or is not in use.
	 */
	unsigned int p2p_expire_poll;

	/**
	 * p2p_neg_conf_ack - check TX status of NEG-CONF
	 */ 
	int p2p_neg_conf_ack;
	...
	/**
	 * persistent_reconnect - Persistent-Reconnect is supported
	 */
	 ...
	 *	group)
	 * @authorized: Variable to check authorized peer
	 ...
	 */
	void (*invitation_result)(void *ctx, int status, const u8 *bssid, int *op_freq);
...
const struct p2p_peer_info * p2p_wfd_get_peer_info(struct p2p_data *p2p, const u8 *addr);
...
/**
 * p2p_invite_set_auth - Set P2P Device Invite Authorized
 * @p2p: P2P module context from p2p_init()
 * @peer: Device Address of the peer P2P Device
 * @auth: Authorized
 * Returns: 0 on success, -1 on failure
...
/**
 * p2p_get_own_info - Get P2P device/if information in text format
 * @p2p: P2P module context from p2p_init()
 * @buf: Buffer for returning text
 * @buflen: Maximum buffer length
 * Returns: Number of octets written to the buffer or -1 on failure
...
/**
 * p2p_set_acs_state - To set acs state
 * @p2p: P2P module context from p2p_init()
 * @acs_state: ACS state (0: disabled, 1: enabled)
 */
int p2p_set_acs_state(struct p2p_data *p2p, int acs_state);


/**
 * p2p_dump_rssi - dump the current RSSI records
 * @p2p: P2P module context from p2p_init()
 */
...
/**
 * p2p_set_rssi_thr - Set the RSSI Threshold for P2P Connection
 * @p2p: P2P module context from p2p_init()
 */
...
/**
 * p2p_clean_rssi - Clean the RSSI values of all p2p devices
 * @p2p: P2P module context from p2p_init()
 */
void p2p_clean_rssi(struct p2p_data *p2p);

int p2p_check_prov_pending(struct p2p_data *p2p);
#endif /* P2P_H */
