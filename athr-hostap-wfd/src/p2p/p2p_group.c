				     ...
				     struct wpabuf *ie)
{
	...
	if (group->p2p->group_limit_reached)
		group_capab |= P2P_GROUP_CAPAB_GROUP_LIMIT;
#ifdef CONFIG_WIFI_DISPLAY			
	if (p2p->p2p_wifi_display && p2p->p2p_diff_attr_wfd)
		p2p_buf_add_capability(ie, p2p->p2p_dev_capab_wfd, group_capab);
	else
...
void p2p_group_set_beacon_update(struct p2p_group *group, u8 beacon_update)
{
	group->beacon_update = beacon_update;
}
			  ...
			  const u8 *ie, size_t len)
{
	...
	if (m->p2p_ie) {
		struct p2p_message msg;
		if(p2p_parse_p2p_ie(m->p2p_ie, &msg)) {
			return 0;		
		}

		#ifdef CONFIG_WIFI_DISPLAY
		if(m->wfd_ie)
			msg.wfd_subelems = m->wfd_ie;
		#endif

		if(msg.p2p_device_addr != NULL) {
			struct p2p_device *dev;
			dev = p2p_get_device(group->p2p, msg.p2p_device_addr);
			if(dev == NULL) {
				p2p_add_dev_from_sta_connected(group->p2p,
								msg.p2p_device_addr,
								&msg);
			}
			//
		...
		struct p2p_device *dev = p2p_get_device(group->p2p, addr);
		if (dev != NULL ) {
			if (dev->info.wfd_subelems != NULL) {
...
const int p2p_group_get_persistent_group(struct p2p_group *group)
{
	return group->cfg->persistent_group;
}


const unsigned int p2p_group_get_max_clients(struct p2p_group *group)
{
	return group->cfg->max_clients;
}


const unsigned int p2p_group_get_num_members(struct p2p_group *group)
{
	return group->num_members;
}


const int p2p_group_get_group_formation(struct p2p_group *group)
{
	return group->group_formation;
}


const u8 *p2p_group_get_member_dev_addr(struct p2p_group *group, const u8 *addr)
{
	struct p2p_group_member *m = group->members;
...
int p2p_group_set_group_limit(struct p2p_group *group, int group_limit)
{
	if (!group)
		return -1;     

	wpa_printf(MSG_DEBUG, "P2P: %s group limit bit",group_limit==1?"Set":"Clear");
	group->p2p->group_limit_reached = group_limit;
