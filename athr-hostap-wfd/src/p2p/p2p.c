...
static void p2p_expire_peers(struct p2p_data *p2p)
{
	...
	dl_list_for_each_safe(dev, n, &p2p->devices, struct p2p_device, list) {
		if (dev->last_seen.sec + p2p->cfg->p2p_expire_age >= now.sec)
			continue;
...
static void p2p_expiration_timeout(void *eloop_ctx, void *timeout_ctx)
{
	...
	eloop_register_timeout(p2p->cfg->p2p_expire_poll, 0,
...
static void p2p_listen_in_find(struct p2p_data *p2p)
{
	...
	ies = p2p_build_probe_resp_ies(p2p, NULL);
	if (ies == NULL)
		return;
					     ...
					     const u8 *addr)
{
	...
	p2p_update_device_rssi(p2p, addr, p2p->rssi);
	p2p->rssi = 0;
				 ...
				 const u8 *gi, size_t gi_len)
{
	...
	for (c = 0; c < info.num_clients; c++) {
		...
		if (dev) {
			...
			p2p_copy_client_info(dev, cli);
			dev->oper_freq = freq;
			if (P2P_DEV_RSSI_OVER_THR(p2p, dev)) {
...
int p2p_add_device(struct p2p_data *p2p, const u8 *addr, int freq, int level,
		   const u8 *ies, size_t ies_len, int scan_res, u8 come_from)
{
	struct p2p_device *dev;
	...
	if (os_memcmp(addr, p2p_dev_addr, ETH_ALEN) != 0) {
		...
		wpa_msg(p2p->cfg->msg_ctx, MSG_DEBUG,"%s[%d]%02x:%02x:%02x\n\r",__func__,__LINE__,dev->interface_addr[3],dev->interface_addr[4],dev->interface_addr[5]);
	} else if(msg.capability != NULL) {
		if(msg.capability[1] & P2P_GROUP_CAPAB_GROUP_OWNER) {			
			os_memcpy(dev->interface_addr, addr, ETH_ALEN);
			wpa_msg(p2p->cfg->msg_ctx, MSG_DEBUG,"%s[%d]%02x:%02x:%02x\n\r",__func__,__LINE__,dev->interface_addr[3],dev->interface_addr[4],dev->interface_addr[5]);
		}
	} 
	...
	if (msg.wfd_subelems) {
		...
		switch (come_from) {
			case P2P_ADD_DEVICE_FROM_SCAN_RESULT:
				if (msg.device_id) {
					dev->info.wfd_subelems_come_from[0] = WLAN_FC_STYPE_BEACON;
					dev->info.wfd_subelems_come_from[1] = 0;
					wpabuf_free(dev->info.wfd_subelems_from[WFD_IE_FROM_BEACON]);
					dev->info.wfd_subelems_from[WFD_IE_FROM_BEACON] = wpabuf_dup(msg.wfd_subelems);
					wpa_msg(p2p->cfg->msg_ctx, MSG_DEBUG, "P2P: Save WFD-IE from "
					        "Beacon frame");
				} else {
					dev->info.wfd_subelems_come_from[0] = WLAN_FC_STYPE_PROBE_RESP;
					dev->info.wfd_subelems_come_from[1] = 0;
					wpabuf_free(dev->info.wfd_subelems_from[WFD_IE_FROM_PROBE_RESP]);
					dev->info.wfd_subelems_from[WFD_IE_FROM_PROBE_RESP] = wpabuf_dup(msg.wfd_subelems);
					wpa_msg(p2p->cfg->msg_ctx, MSG_DEBUG, "P2P: Save WFD-IE from "
					        "Probe-Response frame");
				}
				break;
			case P2P_ADD_DEVICE_FROM_INVITATION:
				dev->info.wfd_subelems_come_from[0] = WLAN_FC_STYPE_ACTION;
				dev->info.wfd_subelems_come_from[1] = P2P_INVITATION_REQ;
				wpabuf_free(dev->info.wfd_subelems_from[WFD_IE_FROM_INVI_REQ]);
				dev->info.wfd_subelems_from[WFD_IE_FROM_INVI_REQ] = wpabuf_dup(msg.wfd_subelems);
				wpa_msg(p2p->cfg->msg_ctx, MSG_DEBUG, "P2P: Save WFD-IE from "
				        "Invitation Request frame");
				break;
			case P2P_ADD_DEVICE_FROM_PROVISION:
				dev->info.wfd_subelems_come_from[0] = WLAN_FC_STYPE_ACTION;
				dev->info.wfd_subelems_come_from[1] = P2P_PROV_DISC_REQ;
				wpabuf_free(dev->info.wfd_subelems_from[WFD_IE_FROM_PROV_REQ]);
				dev->info.wfd_subelems_from[WFD_IE_FROM_PROV_REQ] = wpabuf_dup(msg.wfd_subelems);
				wpa_msg(p2p->cfg->msg_ctx, MSG_DEBUG, "P2P: Save WFD-IE from "
				        "Provision Request frame");
				break;
		}
	}
...
static void p2p_device_free(struct p2p_data *p2p, struct p2p_device *dev)
{
	...
	wpabuf_free(dev->info.wfd_subelems);
	os_memset(dev->info.wfd_subelems_come_from, 0xff, 2);
	for (i = 0; i < MAX_FRAME_TYPE; i++)
		wpabuf_free(dev->info.wfd_subelems_from[i]);
...
static int p2p_get_next_prog_freq(struct p2p_data *p2p)
{
	...
	for (cl = 0; cl < c->reg_classes; cl++) {
		...
		/* FIX : should not use reg_class to check channel band. */
		if((p2p->cfg->p2p_scan_band == 1) && (cla->reg_class > 100))
			continue;
		else if((p2p->cfg->p2p_scan_band == 2) && (cla->reg_class < 100))
			continue;
...
static int p2p_prepare_channel(struct p2p_data *p2p, unsigned int force_freq)
{
	if (force_freq) {
		...
		if (p2p->cfg->p2p_scan_band == 1) {
			if (!p2p->cfg->cfg_op_channel && p2p->best_freq_24 > 0 &&
		...
		} else if (p2p->cfg->p2p_scan_band == 2) {
			if (!p2p->cfg->cfg_op_channel && p2p->best_freq_5 > 0 &&
		...
		if (p2p->cfg->p2p_scan_band) {
			struct p2p_channels res;
			p2p_channels_skip(&p2p->channels,
							&res, p2p->cfg->p2p_scan_band);
			os_memcpy(&p2p->channels, &res,
				  sizeof(struct p2p_channels));
		}
	}
	wpa_msg(p2p->cfg->msg_ctx, MSG_DEBUG,
		...
		int pd_before_go_neg)
{
	...
	if (p2p->persistent_ssid_len)
	{
	    wpa_msg(p2p->cfg->msg_ctx, MSG_DEBUG,
		"P2P: p2p_connect with Persistent GO");
	    p2p->ssid_set = 1;
	}
	else
	{
	    wpa_msg(p2p->cfg->msg_ctx, MSG_DEBUG,
		"P2P: p2p_connect without Persistent GO");
	    p2p->ssid_set = 0;
	}
...
void p2p_persistent_set(struct p2p_data *p2p, u8 id, u8 *ssid,
		size_t ssid_len)
{
	wpa_msg(p2p->cfg->msg_ctx, MSG_DEBUG,
		"P2P: Set Persistent Group, id=%d, ssid=%s", id, ssid);
	p2p->persistent_id = id;
    os_memcpy(p2p->persistent_ssid, ssid, ssid_len);
    p2p->persistent_ssid_len = ssid_len;
}
		      ...
		      struct p2p_device *dev, struct p2p_message *msg)
{
	...
	if (msg->wfd_subelems) {
		...
		dev->info.wfd_subelems = wpabuf_dup(msg->wfd_subelems);
		dev->info.wfd_subelems_come_from[0] = WLAN_FC_STYPE_ACTION;
		dev->info.wfd_subelems_come_from[1] = P2P_GO_NEG_REQ;
		wpabuf_free(dev->info.wfd_subelems_from[WFD_IE_FROM_GO_NEG_REQ]);
		dev->info.wfd_subelems_from[WFD_IE_FROM_GO_NEG_REQ] = wpabuf_dup(msg->wfd_subelems);
		wpa_msg(p2p->cfg->msg_ctx, MSG_DEBUG, "P2P: Save WFD-IE from "
				       ...
				       const u8 *ie, size_t ie_len)
{
	...
	if (dev) {
		...
		if (msg.wfd_subelems) {
			wpabuf_free(dev->info.wfd_subelems);
			dev->info.wfd_subelems = wpabuf_dup(msg.wfd_subelems);
			dev->info.wfd_subelems_come_from[0] = WLAN_FC_STYPE_PROBE_REQ;
			dev->info.wfd_subelems_come_from[1] = 0;
			wpabuf_free(dev->info.wfd_subelems_from[WFD_IE_FROM_PROBE_REQ]);
			dev->info.wfd_subelems_from[WFD_IE_FROM_PROBE_REQ] = wpabuf_dup(msg.wfd_subelems);
			wpa_msg(p2p->cfg->msg_ctx, MSG_DEBUG, "-P2P: Save WFD-IE from "
				"Probe-Request frame");
		} else {
			wpabuf_free(dev->info.wfd_subelems);
			dev->info.wfd_subelems = NULL;
			wpa_msg(p2p->cfg->msg_ctx, MSG_DEBUG, "--P2P: Remove WFD-IE from "
				"Probe-Request frame");			
		}
		#endif
		os_get_time(&dev->last_seen);
...
struct p2p_device * p2p_add_dev_from_sta_connected(struct p2p_data *p2p,
						const u8 *addr,
						struct p2p_message *msg)
{
	struct p2p_device *dev;
	...
	if (dev) {
		...
		if (msg->wfd_subelems) {
			wpabuf_free(dev->info.wfd_subelems);
			dev->info.wfd_subelems = wpabuf_dup(msg->wfd_subelems);
			dev->info.wfd_subelems_come_from[0] = WLAN_FC_STYPE_REASSOC_REQ;
			dev->info.wfd_subelems_come_from[1] = 0;
			wpabuf_free(dev->info.wfd_subelems_from[WFD_IE_FROM_ASSOC_REQ]);
			dev->info.wfd_subelems_from[WFD_IE_FROM_ASSOC_REQ] = wpabuf_dup(msg->wfd_subelems);
			wpa_msg(p2p->cfg->msg_ctx, MSG_DEBUG, "-P2P: Save WFD-IE from "
	...
	if (os_memcmp(dev->interface_addr, addr, ETH_ALEN) != 0) {
		os_memcpy(dev->interface_addr, addr, ETH_ALEN);
		wpa_msg(p2p->cfg->msg_ctx, MSG_DEBUG,"%s[%d]%02x:%02x:%02x:%02x:%02x:%02x\n\r",__func__,__LINE__,dev->interface_addr[0],dev->interface_addr[1],dev->interface_addr[2],dev->interface_addr[3],dev->interface_addr[4],dev->interface_addr[5]);
	} else if(msg->capability != NULL) {
		if(msg->capability[1] & P2P_GROUP_CAPAB_GROUP_OWNER) {			
			os_memcpy(dev->interface_addr, addr, ETH_ALEN);
			wpa_msg(p2p->cfg->msg_ctx, MSG_DEBUG,"%s[%d]%02x:%02x:%02x\n\r",__func__,__LINE__,dev->interface_addr[3],dev->interface_addr[4],dev->interface_addr[5]);
...
struct wpabuf * p2p_build_probe_resp_ies(struct p2p_data *p2p, const u8 *addr)
{
	struct wpabuf *buf;
	u8 *len;
	int pw_id = -1;
	...
	if (p2p->wfd_ie_probe_resp) {
		...
		if (addr) {
			...
			if (dev && !dev->info.wfd_subelems) {
				wpa_msg(p2p->cfg->msg_ctx, MSG_DEBUG, "P2P-Device: " MACSTR "\n", MAC2STR(addr));
				add_wfd_ie = 0;
			}
		}

		if (add_wfd_ie || addr == NULL)
			wpabuf_put_buf(buf, p2p->wfd_ie_probe_resp);
	}

	/* P2P IE */
	len = p2p_buf_add_ie_hdr(buf);
	if (p2p->p2p_wifi_display && p2p->p2p_diff_attr_wfd)
		p2p_buf_add_capability(buf, p2p->p2p_dev_capab_wfd &
		...
		const u8 *bssid, const u8 *ie, size_t ie_len)
{
	 ...
	 * parameters.
	 */
	ies = p2p_build_probe_resp_ies(p2p, addr);
	if (ies == NULL)
		return P2P_PREQ_NOT_PROCESSED;
...
#ifdef CONFIG_WIFI_DISPLAY	
	if (p2p->p2p_wifi_display && p2p->p2p_diff_attr_wfd)
		p2p_buf_add_capability(tmp, p2p->p2p_dev_capab_wfd, group_capab);
	else
#endif /* CONFIG_WIFI_DISPLAY */			
...
#ifdef CONFIG_WIFI_DISPLAY	
	if (p2p->p2p_wifi_display && p2p->p2p_diff_attr_wfd)
		p2p_buf_add_capability(tmp, p2p->p2p_dev_capab_wfd, 0);
	else
#endif /* CONFIG_WIFI_DISPLAY */		
...
#ifdef CONFIG_WIFI_DISPLAY	
	p2p->p2p_dev_capab_wfd = p2p->dev_capab;
	p2p->p2p_wifi_display = 1;
#endif /* CONFIG_WIFI_DISPLAY */		
...
void p2p_deinit(struct p2p_data *p2p)
{
	...
	wpabuf_free(p2p->wfd_ie_invitation_req);
	wpabuf_free(p2p->wfd_ie_invitation_resp);
	wpabuf_free(p2p->wfd_ie_prov_disc_req);
	wpabuf_free(p2p->wfd_ie_prov_disc_resp);
	wpabuf_free(p2p->wfd_ie_go_neg_req);
			 ...
			 int level, const u8 *ies, size_t ies_len)
{
	p2p_add_device(p2p, bssid, freq, level, ies, ies_len, 1,
	               P2P_ADD_DEVICE_FROM_SCAN_RESULT);
...
#ifdef CONFIG_WIFI_DISPLAY	
	if (p2p->p2p_wifi_display && p2p->p2p_diff_attr_wfd)
		p2p_buf_add_capability(ies, p2p->p2p_dev_capab_wfd &
		               ~P2P_DEV_CAPAB_CLIENT_DISCOVERABILITY, 0);
	else
#endif /* CONFIG_WIFI_DISPLAY */		
			       ...
			       enum p2p_send_action_result result)
{
	...
	if ((result == P2P_SEND_ACTION_NO_ACK) &&
		(p2p->cfg->p2p_neg_conf_ack)) {            
		if (p2p->go_neg_peer){
			wpa_msg(p2p->cfg->msg_ctx, MSG_ERROR,
				"P2P: GO Negotiation Confirm TX Fail!");

			p2p->cfg->send_action_done(p2p->cfg->cb_ctx);
			p2p->go_neg_peer->go_state = UNKNOWN_GO;
			p2p->start_after_scan = P2P_AFTER_SCAN_NOTHING;
			...
			enum p2p_send_action_result result)
{
	...
	case P2P_PENDING_GO_DISC_REQ:
		...
		break;
	case P2P_PENDING_PROV_DISC_REQ:
		p2p->pending_action_state = P2P_NO_PENDING_ACTION;
		break;	
	}
}
...
{
	p2p->cfg->send_action_done(p2p->cfg->cb_ctx);
	#if 0//known issue,remove it to pass internal p2p test
	if (p2p->go_neg_peer &&
		...
		if (p2p->invite_peer) {
			...
			if (p2p->cfg->invitation_result)
				p2p->cfg->invitation_result(
					p2p->cfg->cb_ctx, -1, NULL, &p2p->inv_op_freq);
		}
		p2p_set_state(p2p, P2P_IDLE);
	}
}
...
{
	if ((len == 2) && (postfix[0] == '\"') && (postfix[1] == '\"')) {
		os_memset(postfix, 0, len);
		len = 0;
	}
...
int p2p_get_own_info(struct p2p_data *p2p, char *buf, size_t buflen)
{
	int res;
	char *pos, *end;
	...
	res = os_snprintf(pos, end - pos,
	                  ...
	                  p2p->cfg->dev_name,
	                  wps_dev_type_bin2str(p2p->cfg->pri_dev_type, devtype, sizeof(devtype)),
	...
	if (p2p->num_groups > 0) {
		for (g = 0; g < p2p->num_groups; g++) {
			struct p2p_group *group = p2p->groups[g];

			if (group) {
				u8 group_capab = 0;
				group_capab |= P2P_GROUP_CAPAB_GROUP_OWNER;             //bit_0.
				if (p2p_group_get_persistent_group(group))
					group_capab |= P2P_GROUP_CAPAB_PERSISTENT_GROUP;    //bit_1.
				if (p2p_group_get_num_members(group) >= p2p_group_get_max_clients(group))
					group_capab |= P2P_GROUP_CAPAB_GROUP_LIMIT;         //bit_2.
				if (p2p->cfg->p2p_intra_bss)
					group_capab |= P2P_GROUP_CAPAB_INTRA_BSS_DIST;      //bit_3.
				if (p2p->cross_connect)
					group_capab |= P2P_GROUP_CAPAB_CROSS_CONN;          //bit_4.
				if (p2p->cfg->persistent_reconnect)
					group_capab |= P2P_GROUP_CAPAB_PERSISTENT_RECONN;   //bit_5
				if (p2p_group_get_group_formation(group))
					group_capab |= P2P_GROUP_CAPAB_GROUP_FORMATION;     //bit_6.
			    res = os_snprintf(pos, end - pos,
				                  "p2p_if_addr=" MACSTR "\n"
				                  "group_capab=0x%x\n\n",
				                  MAC2STR(p2p_group_get_interface_addr(group)),
				                  group_capab);
				if (res < 0 || res >= end - pos)
					return pos - buf;
				pos += res;
...
static void p2p_update_wfd_ie_groups(struct p2p_data *p2p, u8 beacon_update)
{
	size_t g;
	...
	for (g = 0; g < p2p->num_groups; g++) {
		group = p2p->groups[g];
		p2p_group_set_beacon_update(group, beacon_update);
...
int p2p_set_wfd_ie_beacon(struct p2p_data *p2p, struct wpabuf *ie)
{
	...
	p2p_update_wfd_ie_groups(p2p, 1);
	return 0;
}
...
int p2p_set_wfd_ie_probe_resp(struct p2p_data *p2p, struct wpabuf *ie)
{
	...
	p2p_update_wfd_ie_groups(p2p, 0);
	return 0;
}
...
int p2p_set_wfd_ie_invitation_req(struct p2p_data *p2p, struct wpabuf *ie)
{
	wpabuf_free(p2p->wfd_ie_invitation_req);
	p2p->wfd_ie_invitation_req = ie;
	return 0;
}


int p2p_set_wfd_ie_invitation_resp(struct p2p_data *p2p, struct wpabuf *ie)
{
	wpabuf_free(p2p->wfd_ie_invitation_resp);
	p2p->wfd_ie_invitation_resp = ie;
	return 0;
}
...
int p2p_set_wfd_ie_go_neg_req(struct p2p_data *p2p, struct wpabuf *ie)
{
	wpabuf_free(p2p->wfd_ie_go_neg_req);
	p2p->wfd_ie_go_neg_req = ie;
	return 0;
}
...
int p2p_get_peer_wfd_ie(struct p2p_data *p2p,
                        u8 *addr, u8 frame_type, char *buf)
{
	struct p2p_device *dev;
	int res;
	char *pos, *end;
	...
	dev = p2p_get_device(p2p, addr);
	if (dev) {
		if (dev->info.wfd_subelems_from[frame_type]) {
			res = os_snprintf(pos, end - pos, "wfd_subelems_from=");
			if (res < 0 || res >= end - pos)
				return pos - buf;
			pos += res;

			pos += wpa_snprintf_hex(pos, end - pos,
						wpabuf_head(dev->info.wfd_subelems_from[frame_type]),
						wpabuf_len(dev->info.wfd_subelems_from[frame_type]));
...
int p2p_set_peer_wfd_ie(struct p2p_data *p2p,
                        u8 *addr, u8 frame_type, char *buf, int buf_len)
{
	struct p2p_device *dev;

	dev = p2p_get_device(p2p, addr);
	if (dev) {
	    wpabuf_free(dev->info.wfd_subelems_from[0]);
		wpabuf_free(dev->info.wfd_subelems_from[frame_type]);
		if (buf) {
			dev->info.wfd_subelems_from[frame_type] =
				ieee802_11_vendor_ie_concat(buf, buf_len, WFD_IE_VENDOR_TYPE);
			if (frame_type == WFD_IE_FROM_ASSOC_REQ || frame_type == WFD_IE_FROM_ASSOC_RESP) {
				dev->info.wfd_subelems_from[0] = wpabuf_dup(dev->info.wfd_subelems_from[frame_type]);
			} else {
				dev->info.wfd_subelems_from[0] = NULL;
			}
		} else {
			dev->info.wfd_subelems_from[frame_type] = NULL;
			dev->info.wfd_subelems_from[0] = NULL;
		}
...
static int p2p_wfd_device_type_get(const struct wpabuf *wfd)
{
	const u8 *pos, *end;
	u8 subelem;
	u16 len, info;
...
#define WFD_DEVICE_TYPE_SOURCE              0
#define WFD_DEVICE_TYPE_PRIMARY_SINK        1
#define WFD_DEVICE_TYPE_SECONDARY_SINK      2
#define WFD_DEVICE_TYPE_SOURCE_PRIMARY_SINK 3
int p2p_wfd_device_type_check(struct p2p_data *p2p, const struct wpabuf *wfd)
{
	int my_dev_type, peer_dev_type;
	int ret = 0;

	if (p2p->wfd_dev_info == NULL) {
		wpa_msg(p2p->cfg->msg_ctx, MSG_DEBUG, "WFD-IE Device Information Not Found");
		ret = 0;
	} else {
		my_dev_type = p2p_wfd_device_type_get(p2p->wfd_dev_info);
		peer_dev_type = p2p_wfd_device_type_get(wfd);
		wpa_msg(p2p->cfg->msg_ctx, MSG_DEBUG, "my_dev_type = %d, peer_dev_type = %d",
		        my_dev_type, peer_dev_type);
		if ((my_dev_type == -1) || (peer_dev_type == -1)) {
			wpa_msg(p2p->cfg->msg_ctx, MSG_DEBUG, "WFD-IE Device Type Not Found");
			ret = 0;
		} else {
			switch (my_dev_type) {
				case WFD_DEVICE_TYPE_SOURCE:
					if (peer_dev_type == WFD_DEVICE_TYPE_SOURCE)
						ret = -1;
					break;
				case WFD_DEVICE_TYPE_PRIMARY_SINK:
				case WFD_DEVICE_TYPE_SECONDARY_SINK:
					if ((peer_dev_type == WFD_DEVICE_TYPE_PRIMARY_SINK) ||
					    (peer_dev_type == WFD_DEVICE_TYPE_SECONDARY_SINK))
						ret = -1;
					break;
				case WFD_DEVICE_TYPE_SOURCE_PRIMARY_SINK:
					ret = 0;
					break;
			}
		}
...
void p2p_wifi_display_set(struct p2p_data *p2p, u8 enabled)
{
	p2p->p2p_wifi_display = enabled;
	wpa_msg(p2p->cfg->msg_ctx, MSG_DEBUG, "P2P: p2p_wifi_display = %d\n",
	        p2p->p2p_wifi_display);
}


void p2p_wfd_set_diff_attr(struct p2p_data *p2p, u8 enabled)
{
	p2p->p2p_diff_attr_wfd = enabled;
	wpa_msg(p2p->cfg->msg_ctx, MSG_DEBUG, "P2P: p2p_diff_attr_wfd = %d\n",
	        p2p->p2p_diff_attr_wfd);
}


void p2p_wfd_set_dev_capab(struct p2p_data *p2p, u8 bitmap, u8 enabled)
{
	p2p->p2p_dev_capab_wfd &= (~BIT(bitmap));
	p2p->p2p_dev_capab_wfd |= (enabled << bitmap);
	wpa_msg(p2p->cfg->msg_ctx, MSG_DEBUG, "P2P: p2p_dev_capab_wfd = %2.2x\n",
	        p2p->p2p_dev_capab_wfd);
}


int p2p_wfd_get_dev_capab(struct p2p_data *p2p, char *buf)
{
	int res;
	char *pos, *end;
	...
	res = os_snprintf(pos, end - pos, "p2p_dev_capab_wfd=0x%x\n\n", p2p->p2p_dev_capab_wfd);
	pos += res;
	res = os_snprintf(pos, end - pos, "\n");
	return pos - buf;
}
...
const struct p2p_peer_info *
p2p_wfd_get_peer_info(struct p2p_data *p2p, const u8 *addr)
{
	struct p2p_device *dev;
...
int p2p_set_acs_state(struct p2p_data *p2p, int acs_state)
{
	p2p->acs_state = acs_state;

	return 1;
}


int p2p_dev_addr_get_from_ie(u8 *addr, u8 *ies, size_t ies_len)
{
	struct p2p_message msg;
	int ret = 0;
	...
	if (msg.p2p_device_addr) {
		memcpy(addr, msg.p2p_device_addr, 6);
	} else if (msg.device_id) {
		memcpy(addr,msg.device_id,6);
	} else {
		ret = 1;
	}
...
void p2p_update_device_rssi(struct p2p_data *p2p, const u8 *addr, int rssi)
{
	struct p2p_device *dev;
	int dev_found = 0;
	...
	if (dev_found) {
		if (!dev->probe_rssi)
			...
			dev->probe_rssi = (rssi >> 2) + ((dev->probe_rssi * 3) >> 2);

		dev->last_probe_rssi = rssi;
		dev->received ++;
		dev->rssi_history[dev->rssi_history_index] = rssi;
		dev->rssi_history_index = (dev->rssi_history_index + 1) % 10;
	} else
		p2p->rssi = rssi;
...
int p2p_dump_rssi(struct p2p_data *p2p)
{
	struct p2p_device *dev;

	if (p2p->p2p_rssi_thr) {
		wpa_printf(MSG_ERROR, "====================================");
		wpa_printf(MSG_ERROR, "The setting of RSSI Threshold (%02d)",p2p->p2p_rssi_thr);
		wpa_printf(MSG_ERROR, "====================================");
	}

	wpa_printf(MSG_ERROR, "========= Print RSSI table =========");
	dl_list_for_each(dev, &p2p->devices, struct p2p_device, list) {
		if (dev->probe_rssi) {
			wpa_printf(MSG_ERROR, "MAC = [%02x:%02x:%02x:%02x:%02x:%02x], RX = %04d, AVG_RSSI = %03d, LAST_RSSI = %03d", 
						dev->info.p2p_device_addr[0],
						dev->info.p2p_device_addr[1],
						dev->info.p2p_device_addr[2],
						dev->info.p2p_device_addr[3],
						dev->info.p2p_device_addr[4],
						dev->info.p2p_device_addr[5],
						dev->received,
						(dev->probe_rssi >> 2),
						(dev->last_probe_rssi >> 2));
			wpa_printf(MSG_ERROR, "   HISTORY = [%03d, %03d, %03d, %03d, %03d, %03d, %03d, %03d, %03d, %03d]\n", 
						dev->rssi_history[(dev->rssi_history_index) % 10] >> 2,
						dev->rssi_history[(dev->rssi_history_index + 1) % 10] >> 2,
						dev->rssi_history[(dev->rssi_history_index + 2) % 10] >> 2,
						dev->rssi_history[(dev->rssi_history_index + 3) % 10] >> 2,
						dev->rssi_history[(dev->rssi_history_index + 4) % 10] >> 2,
						dev->rssi_history[(dev->rssi_history_index + 5) % 10] >> 2,
						dev->rssi_history[(dev->rssi_history_index + 6) % 10] >> 2,
						dev->rssi_history[(dev->rssi_history_index + 7) % 10] >> 2,
						dev->rssi_history[(dev->rssi_history_index + 8) % 10] >> 2,
						dev->rssi_history[(dev->rssi_history_index + 9) % 10] >> 2);
		}
	}
	wpa_printf(MSG_ERROR, "========= RSSI table end =========");
  
	return 1;
}


void p2p_set_rssi_thr(struct p2p_data *p2p, int rssi_thr)
{
	p2p->p2p_rssi_thr = rssi_thr;
}


void p2p_clean_rssi(struct p2p_data *p2p)
{
    	struct p2p_device *dev;
	...
	dl_list_for_each(dev, &p2p->devices, struct p2p_device, list) {
		if (dev->probe_rssi) {
			dev->probe_rssi = 0;
			dev->received = 0;
			dev->last_probe_rssi = 0;
			dev->rssi_history_index = 0;

			os_memset(dev->rssi_history, 0, sizeof(dev->rssi_history));            
		}
	}
}
