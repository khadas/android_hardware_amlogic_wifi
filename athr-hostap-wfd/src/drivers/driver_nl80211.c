...
static int wpa_driver_nl80211_probe_resp_report(void *priv, int report);
...
static int
set80211priv(void *priv, int op, void *data, int len, int show_err);
			    ...
			    const u8 *frame, size_t len)
{
	...
	if (stype == WLAN_FC_STYPE_ACTION) {
		...
		event.rx_action.len = frame + (len -4)- event.rx_action.data;
		memcpy(&event.rx_action.rssi, frame + len - 4, 4);

		wpa_supplicant_event(drv->ctx, EVENT_RX_ACTION, &event);
	} else {
		event.rx_mgmt.frame = frame;
		event.rx_mgmt.frame_len = len-4;
		memcpy(&event.rx_mgmt.ssi_signal, frame + len - 4, 4);
                                 ...
                                 int cmd, struct nlattr **tb)
{
	...
	case NL80211_CMD_PROBE_CLIENT:
		...
		break;
	case NL80211_CMD_ACL_REJECT_INFO: {
		union wpa_event_data data;
		char *data_ptr = nla_data(tb[NL80211_ATTR_FRAME]);
		os_memset(&data, 0, sizeof(data));
		os_memcpy(data.acl_reject_info.addr, data_ptr, ETH_ALEN);
		wpa_supplicant_event(drv->ctx, EVENT_ACL_REJECT, &data);
		break;
	}
	default:
...
static int wpa_driver_nl80211_init_nl_global(struct nl80211_global *global)
{
	...
	ret = nl_get_multicast_id(global, "nl80211", "athr");
	if (ret >= 0)
		ret = nl_socket_add_membership(global->nl_event, ret);
	if (ret < 0) {
		wpa_printf(MSG_ERROR, "nl80211: Could not add multicast "
			   "membership for athr events: %d (%s)",
			   ret, strerror(-ret));
		goto err;
	}
...
static void wpa_driver_nl80211_deinit(void *priv)
{
	...
	if (bss->nl_presp)
		wpa_driver_nl80211_probe_resp_report(bss, 0);
...
nla_put_failure:
	...
	if(ret == -EBUSY) {
		wpa_printf(MSG_DEBUG,"%s[%d]retry_cnt=%d\n\r",__func__,__LINE__,retry_cnt);
		if(retry_cnt < 50) {
			os_sleep(0, 20000);
			retry_cnt++;
			goto retry_label;			
...
#define ATH_DEFAULT_NOISE_FLOOR     -95
	...
	if (bss[NL80211_BSS_SIGNAL_MBM]) {
		...
		r->noise = ATH_DEFAULT_NOISE_FLOOR; /* set the noise level */
		if (r->level < r->noise)
			r->level = 0;
		else
			r->level -= r->noise; /* convert rssi from dBm to db */
		r->flags |= WPA_SCAN_QUAL_INVALID;
	} else if (bss[NL80211_BSS_SIGNAL_UNSPEC]) {
		r->level = nla_get_u8(bss[NL80211_BSS_SIGNAL_UNSPEC]);
		r->flags |= WPA_SCAN_QUAL_INVALID | WPA_SCAN_NOISE_INVALID;
	} else
		r->flags |= WPA_SCAN_LEVEL_INVALID | WPA_SCAN_QUAL_INVALID | WPA_SCAN_NOISE_INVALID;
...
static int athr_driver_get_best_channel(struct i802_bss *bss)
{
	...
	if (drv->scanband >= ATHR_CMD_SCANBAND_CHAN_ONLY) {
		/* Do not attempt to get the best channel if doing single channel scan */
		return 0;
	}
                                             ...
                                             unsigned int wait_time)
{
	...
	if (drv->device_ap_sme &&
		...
		if (WLAN_FC_GET_STYPE(fc) == WLAN_FC_STYPE_ACTION) {
			return nl80211_send_frame_cmd(bss, freq, 0,
						data, data_len, &drv->send_action_cookie, 1, noack, 1);
		} else {
			return nl80211_send_frame_cmd(bss, freq, 0,
						data, data_len, NULL, 0, noack, 0);
		}
	}
	...
	struct wpa_driver_associate_params *params)
{
	...
	if ((params->auth_alg & WPA_AUTH_ALG_OPEN) &&
	    (params->pairwise_suite == CIPHER_WEP40 ||
	     params->pairwise_suite == CIPHER_WEP104)) {
		wpa_printf(MSG_DEBUG, "  * Leave out Auth Type for automatic "
			   "selection for OPEN WEP");
		goto skip_auth_type;
	}
...
static int wpa_driver_nl80211_probe_resp_report(void *priv, int report)
{
	...
	/* 
	 * enable probe_req_report even in GO/AP mode since some drivers 
	 * have no monitor ifaces but need the report to reply the 
	 * probe responses
	 */
	if (!is_sta_interface(drv->nlmode) && !drv->device_ap_sme) {
		wpa_printf(MSG_DEBUG, "nl80211: probe_resp_report control only "
	...
	if (!report) {
		if (bss->nl_presp) {
			eloop_unregister_read_sock(
				nl_socket_get_fd(bss->nl_presp));
			nl_destroy_handles(&bss->nl_presp);
		}
		return 0;
	}

	if (bss->nl_presp) {
		wpa_printf(MSG_DEBUG, "nl80211: Probe Response reporting "
			   "already on!");
		return 0;
	}

	bss->nl_presp = nl_create_handle(drv->global->nl_cb, "presp");
	if (bss->nl_presp == NULL)
		return -1;

	if (nl80211_register_frame(bss, bss->nl_presp,
				   (WLAN_FC_TYPE_MGMT << 2) |
				   (WLAN_FC_STYPE_PROBE_RESP << 4),
				   NULL, 0) < 0)
		goto out_err;

	eloop_register_read_sock(nl_socket_get_fd(bss->nl_presp),
	                         wpa_driver_nl80211_event_receive, bss->nl_cb,
	                         bss->nl_presp);
                                     ...
                                     int ifindex, int disabled)
{
	...
	nl80211_cmd(drv, msg, 0, NL80211_CMD_DISABLE_11B_RATES);
	NLA_PUT_U32(msg, NL80211_ATTR_IFINDEX, drv->ifindex);
        NLA_PUT_U8(msg, NL80211_ATTR_DISABLE_11B_RATES, disabled?1:0);
...
static int wpa_driver_nl80211_ap_set_uapsd(void *priv, int enabled) 
{
	struct i802_bss *bss = priv;
	struct wpa_driver_nl80211_data *drv = bss->drv;
        struct nl_msg *msg;
        ...
        nl80211_cmd(drv, msg, 0, NL80211_CMD_AP_SET_UAPSD);
        NLA_PUT_U32(msg, NL80211_ATTR_IFINDEX, drv->ifindex);
        NLA_PUT_U8(msg, NL80211_ATTR_AP_SET_UAPSD, enabled?1:0);
                           

        ret = send_and_recv_msgs(drv, msg, NULL, NULL);
        if (ret) {
                wpa_printf(MSG_DEBUG, "nl80211: AP Set uAPSD failed: ret=%d "
                           "(%s)", ret, strerror(-ret));
        }
...
static int wpa_driver_nl80211_set_tx_sgi(void *priv, u32* mask, u8 threshold)
{
	struct i802_bss *bss = priv;
	struct wpa_driver_nl80211_data *drv = bss->drv;
	struct nl_msg *msg;
	...
	nl80211_cmd(drv, msg, 0, NL80211_CMD_SET_TX_SGI);
	NLA_PUT_U32(msg, NL80211_ATTR_IFINDEX, drv->ifindex);
	NLA_PUT(msg, NL80211_ATTR_SET_TX_SGI_MASK, 8, mask);
	NLA_PUT_U8(msg, NL80211_ATTR_SET_TX_SGI_PER_THRESHOLD, threshold);

	ret = send_and_recv_msgs(drv, msg, NULL, NULL);
	if (ret) {
		wpa_printf(MSG_DEBUG, "nl80211: AP Set TX SGI failed: ret=%d "
		           "(%s)", ret, strerror(-ret));
	}
...
static int nl80211_set_param(void *priv, const char *param)
{
	...
	} else if (os_strstr(param, "p2p_capable=0")) {
		struct i802_bss *bss = priv;
		struct wpa_driver_nl80211_data *drv = bss->drv;
 ...
 */
static int
set80211param_ifname(void *priv, const char *ifname,
                     int op, int arg, int show_err)
{
	...
	if (ioctl(drv->global->ioctl_sock, IEEE80211_IOCTL_SETPARAM, &iwr) < 0) {
		if (show_err)
...
static int
set80211param(void *priv, int op, int arg, int show_err)
{
	struct i802_bss *bss = priv;
	void *drv;

	drv = priv;
	return set80211param_ifname(drv, bss->ifname, op, arg, show_err);
}


static int
call80211ioctl(struct wpa_driver_nl80211_data *drv, int op, struct iwreq *iwr)
{
	wpa_printf(MSG_DEBUG, "%s: %d op=0x%x (%s) subop=0x%x=%d "
	           "value=0x%x,0x%x",
	           __func__, drv->global->ioctl_sock, op,
	           athr_get_ioctl_name(op), iwr->u.mode, iwr->u.mode,
	           iwr->u.data.length, iwr->u.data.flags);
	if (ioctl(drv->global->ioctl_sock, op, iwr) < 0) {
...
static int
get80211param(struct wpa_driver_nl80211_data *drv, char *ifname, int op, int *data)
{
	struct iwreq iwr;
...
static int
set80211priv(void *priv, int op, void *data, int len,
             int show_err)
{
	struct i802_bss *bss = priv;
	struct wpa_driver_nl80211_data *drv = bss->drv;		
	...
	os_memset(&iwr, 0, sizeof(iwr));
	os_strncpy(iwr.ifr_name, bss->ifname, IFNAMSIZ);
...
static int athr_driver_acl(void *priv, const char *cmd,
                           char *buf, size_t buf_len)
{
	struct i802_bss *bss = priv;
	struct wpa_driver_nl80211_data *drv = bss->drv;		
...
#define _MAX_ACL_MACADDR_BUF    (2048)
	...
	if ((param = os_strstr(pos, "maccmd ")) != NULL) {
		...
		ret = set80211param(priv, IEEE80211_PARAM_MACCMD, val, 1);
	} else if (os_strstr(pos, "getmaccmd") != NULL) {
		if (get80211param(drv, bss->ifname, IEEE80211_PARAM_MACCMD,
		                  &i) == 0) {
		...
		if (hwaddr_aton(param, (u8 *) sa.sa_data)) {
			...
			ret = set80211priv(priv, IEEE80211_IOCTL_ADDMAC,
			                   &sa, sizeof(sa), 1);
		...
		if (hwaddr_aton(param, (u8 *) sa.sa_data)) {
			...
			ret = set80211priv(priv, IEEE80211_IOCTL_DELMAC,
			                   &sa, sizeof(sa), 1);
		...
		if (set80211priv(priv, IEEE80211_IOCTL_GET_MACADDR,
 ...
 * @priv: Pointer to i802_bss data from wpa_driver_nl80211_init()
 * @cmd: Command in
			   ...
			   size_t buf_len)
{
	...
	if ((sub_cmd = os_strstr(cmd, "acl ")) != NULL) {
		return athr_driver_acl(priv, sub_cmd + 4, buf, buf_len);
	} else if ((sub_cmd = os_strstr(cmd, "scanmode ")) != NULL) {
		return athr_driver_scanmode(bss, sub_cmd + 9, buf, buf_len);
	}
...
static int athr_driver_kick_sta(void *priv, char *addr)
{
	...
	if (ioctl(drv->global->ioctl_sock, IEEE80211_IOCTL_KICKMAC, &iwr) < 0) {
		return -1;
	}
...
static int athr_driver_set_p2p_max_clients(void *priv, int max_clients)
{
	...
	return set80211param(priv, IEEE80211_PARAM_MAX_CLIENTS, max_clients, 1);
}

static int wpa_driver_nl80211_scan_cancel(void *priv, char *addr)
{
	struct i802_bss *bss = priv;
	struct wpa_driver_nl80211_data *drv = bss->drv;
	struct nl_msg *msg;
...
//	.get_inact_sec = i802_get_inact_sec,
	...
	.probe_resp_report = wpa_driver_nl80211_probe_resp_report,
	.ap_set_uapsd = wpa_driver_nl80211_ap_set_uapsd,
	.set_tx_sgi = wpa_driver_nl80211_set_tx_sgi,
