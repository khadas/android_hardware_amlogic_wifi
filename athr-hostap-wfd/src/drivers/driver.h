...
struct wpa_driver_ops {
	...
	int (*probe_resp_report)(void *priv, int report);

	/**
	 * ap_set_uapsd - Set the uAPSD in AP mode
	 * @priv: Private driver interface data
	 * @enable: Whether uAPSD is enable 
	 * Returns: 0 on success, -1 on failure (or if not supported)
	 *
	 * This command is used to enable/disable uAPSD in AP mode */

	int (*ap_set_uapsd)(void* priv, int enable);

	/**
	 * set_tx_sgi- Set the short GI 
	 * @priv: Private driver interface data
	 * @enable: Whether the short GI is enable
	 * Returns: 0 on success, -1 on failure (or if not supported)
	 *
	 * This command is used to enable/disable short GI */

	int (*set_tx_sgi)(void* priv, u32* mask, u8 threshold);
	...
	/**
	 * kick_sta - to kick out assoicated station
	 * @priv: Private driver interface data
	 * @addr: station mac address
	 * Returns: 0 on success or -1 on failure
	 */
	int (*kick_sta)(void *priv, u8 *addr);
	...
	/**
	 * set_p2p_max_clients - set maximum accepted p2p clients
	 * @priv: Private driver interface data
	 * @max_clients: Maximum p2p client numbers
	 * Returns: 0 on success or -1 on failure
	 */
	int (*set_p2p_max_clients)(void *priv, int max_clients);

	int (*get_scanband)(void *priv, u32 *scanband);
};
...
enum wpa_event_type {
	...
	/**
	 * EVENT_SET_STA_LIMIT - Indicate maximum group limit reached
	 * EVENT_CLEAR_STA_LIMIT - Indicate maximum group limit released
	 */
	EVENT_SET_STA_LIMIT,
	EVENT_CLEAR_STA_LIMIT,

	/**
	 * EVENT_ACL_REJECT - The STA inside ACL reject list is trying to join GO
	 */
	EVENT_ACL_REJECT
};
...
union wpa_event_data {
	...
	struct rx_action {
		...
		/**
		 * rssi - RSSI (in dBm, signal strength) on which the frame was received
		 */
		int rssi;
	} rx_action;
	...
	struct rx_probe_req {
		...
		/**
		 * rssi - RSSI for this probe request
		 */
		int rssi;
	} rx_probe_req;
	...
	/**
	 * struct acl_reject_info - Data for EVENT_ACL_REJECT
	 */
	struct acl_reject_info {
		u8 addr[ETH_ALEN];
	} acl_reject_info;
};
