				      ...
				      struct hostapd_sta_add_params *params)
{
	...
	wpa_msg(drv->ctx, MSG_WARNING, "##### %s, params->set = %d", __func__, params->set);//Carter
				     ...
				     const u8 *addr, int wds)
{
	...
	wpa_printf(MSG_ERROR, "##### nl80211: Create interface iftype %d (%s)",
	...
	size_t data_len, int encrypt, const u8 *own_addr, u32 flags)
{
	...
	wpa_msg(drv->ctx, MSG_ERROR, "##### %s, drv->device_ap_sme = %d, drv->use_monitor = %d", __func__, drv->device_ap_sme, drv->use_monitor);//Carter
					    ...
					    int flags_or, int flags_and)
{
	...
	wpa_msg(drv->ctx, MSG_WARNING, "##### %s", __func__);//Carter
				 ...
				 struct wpa_driver_associate_params *params)
{
	...
	wpa_printf(MSG_ERROR, "#### nl80211: %s)", __func__);
	if (params->p2p) {
