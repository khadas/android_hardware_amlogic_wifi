...
const PS8 mac_file_name = "/sys/class/efuse/mac";
#define MAC_STR_LEN 17

static void hexStr2MACAddr( S8 *hexStr, U8* mac )
{
	U8 i;
	S8 *currHexStrPos = hexStr;
	U8 tempValue;
...
#ifdef AMLOGIC    
    PVOID fd_mac, fd;
    S8 buf_mac[MAC_STR_LEN+1] = "00:00:00:00:00:00";
    U8 mac[6] = {0};
    U8 tmpmac1[6] = {0}; 
    int i = 0;
    int filelen;   
    
		if ((fd_mac = os_fopen (mac_file_name, OS_FOPEN_READ)) != NULL) {
			if (os_fread(buf_mac, 1, MAC_STR_LEN, fd_mac)<MAC_STR_LEN) {
				os_error_printf(CU_MSG_ERROR, (PS8)"wlan_loader: Error in reading file %s \n", mac_file_name);
			} else {
				os_error_printf(CU_MSG_ERROR, (PS8)"wlan_loader: Read efuse mac=%s \n", &buf_mac);
				hexStr2MACAddr(buf_mac, mac);
		//		os_error_printf(CU_MSG_ERROR, (PS8)"wlan_loader: mac=%02x:%02x:%02x:%02x:%02x:%02x \n",
		//									 mac[0],mac[1],mac[2],mac[3],mac[4],mac[5]);
				
				if (!(mac[0]==0 && mac[1]==0 && mac[2]==0 && mac[3]==0 && mac[4]==0 && mac[5]==0))							 
		   	if ( (NULL != eeprom_file_name) &&
		         (fd = fopen ((char*)eeprom_file_name, "rb+")) != NULL)
		    {
		//			os_error_printf(CU_MSG_ERROR, (PS8)"wlan_loader: Write mac %s \n", eeprom_file_name);
					for(i=0;i<6;i++){
				    tmpmac1[5-i] = mac[i];
			    }
					...
					filelen = ftell(fd);
					fseek(fd, 3, SEEK_SET);
			    fwrite(&tmpmac1[0], 1, 4, fd);
			    fseek(fd, 10, SEEK_SET);
			    fwrite(&tmpmac1[4], 1, 2, fd);
					fseek(fd, 0, SEEK_SET);
			    fclose(fd);
		     }
		 	}
	   	...
	   	os_error_printf(CU_MSG_ERROR, (PS8)"wlan_loader: Error in opening file %s \n", mac_file_name);
     }
